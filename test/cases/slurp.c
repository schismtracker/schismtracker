...
static const char expected_result[] = EXPECTEDRESULTP1 EXPECTEDRESULTP2;
static const char expected_result_padded[] = EXPECTEDRESULTP1 EXPECTEDRESULTPADDING EXPECTEDRESULTP2;
...
testresult_t test_slurp_2memstream(void)
{
	...
	ASSERT(slurp_2memstream(&fp, (const uint8_t *)expected_result_padded,
		       (const uint8_t *)expected_result_padded + TEST_SLURP_2SIZE + TEST_SLURP_PADDING,
		       TEST_SLURP_2SIZE)
		>= 0);
...
static testresult_t test_slurp_decompress(
	int (*init)(void), int (*start)(slurp_t *fp), void (*quit)(void), const unsigned char *data, size_t datalen)
{
	testresult_t r;
	slurp_t fp;
	static const unsigned char expected_result_gz[] = {'\037', '\213', '\010', '\010', '\234', '\107', '\333',
		'\150', '\002', '\003', '\145', '\170', '\160', '\145', '\143', '\164', '\145', '\144', '\137', '\162',
		'\145', '\163', '\165', '\154', '\164', '\000', '\113', '\114', '\112', '\126', '\110', '\111', '\115',
		'\123', '\110', '\317', '\310', '\124', '\060', '\064', '\062', '\126', '\060', '\061', '\065', '\123',
		'\060', '\267', '\260', '\344', '\362', '\124', '\310', '\311', '\054', '\113', '\125', '\310', '\314',
		'\123', '\110', '\124', '\110', '\317', '\114', '\314', '\053', '\121', '\110', '\052', '\115', '\316',
		'\116', '\055', '\321', '\343', '\112', '\312', '\111', '\055', '\115', '\317', '\320', '\343', '\002',
		'\000', '\240', '\062', '\045', '\375', '\072', '\000', '\000', '\000'};
...
testresult_t test_slurp_bzip2(void)
{
	...
	return test_slurp_decompress(
		bzip2_init, slurp_bzip2, bzip2_quit, expected_result_bz2, sizeof(expected_result_bz2));
}
#endif
...
testresult_t test_slurp_zstd(void)
{
	...
	return test_slurp_decompress(
		zstd_init, slurp_zstd, zstd_quit, expected_result_zstd, sizeof(expected_result_zstd));
}
#endif
