...
static ssize_t _ISFS_open_r(struct _reent *r, void *fd, const char *path, UNUSED int flags, UNUSED int mode)
{
...
static bool read_recursive(DIR_ENTRY *parent)
{
	...
	if (fileCount > 0) {
		...
		for (fileNum = 0; fileNum < fileCount; fileNum++)
			if (!read_recursive(parent->children + fileNum)) return false;
	}
	return true;
}
...
s32 ISFS_SU()
{
	u32 key = 0;
	return ES_Identify(
		(signed_blob *)certs_bin, sizeof(certs_bin), (signed_blob *)su_tmd_bin, sizeof(su_tmd_bin),
