...
{
	if (invalid_drive_specifier(path))
		return NULL;
	if (strchr(path, ':') != NULL)
		path = strchr(path, ':') + 1;
	DIR_ENTRY *entry;
	...
	if (entry == root && !strcmp(".", pathPosition))
		found = true;
	DIR_ENTRY *dir = entry;
	while (!found && !notFound) {
		...
		if (dirnameLength >= ISFS_MAXPATHLEN)
			return NULL;
		...
		while (fileIndex < dir->fileCount && !found && !notFound) {
			...
			if (dirnameLength == strnlen(entry->name, ISFS_MAXPATHLEN - 1)
				&& !strncasecmp(pathPosition, entry->name, dirnameLength))
				found = true;
			if (found && !is_dir(entry) && nextPathPosition)
				found = false;
			if (!found)
				fileIndex++;
		}
...
static ssize_t _ISFS_open_r(
	struct _reent *r, void *fd, const char *path, SCHISM_UNUSED int flags, SCHISM_UNUSED int mode)
{
...
static const devoptab_t dotab_isfs = {DEVICE_NAME, sizeof(FILE_STRUCT), _ISFS_open_r, _ISFS_close_r, NULL, _ISFS_read_r,
	_ISFS_seek_r, _ISFS_fstat_r, _ISFS_stat_r, NULL, NULL, _ISFS_chdir_r, NULL, NULL, sizeof(DIR_STATE_STRUCT),
	_ISFS_diropen_r, _ISFS_dirreset_r, _ISFS_dirnext_r, _ISFS_dirclose_r, NULL, NULL, NULL, NULL, NULL, NULL};
...
static bool read_recursive(DIR_ENTRY *parent)
{
	...
	if (ret != ISFS_OK) {
		...
		if (fd >= 0) {
			...
			if (ISFS_GetFileStats(fd, &st) == ISFS_OK)
				parent->size = st.file_length;
			ISFS_Close(fd);
		}
		return true;
	}
	...
	if (fileCount > 0) {
		if ((ISFS_MAXPATHLEN * fileCount) > BUFFER_SIZE)
			return false;
...
{
	u32 key = 0;
	return ES_Identify((signed_blob *)certs_bin, sizeof(certs_bin), (signed_blob *)su_tmd_bin, sizeof(su_tmd_bin),
		(signed_blob *)su_tik_bin, sizeof(su_tik_bin), &key);
}
