...
static int _oss_start_stop(UNUSED struct midi_port *p)
{
	return 1; /* do nothing */
}
...
static int _tryopen(int n, const char *name, struct midi_provider *_oss_provider)
{
	...
	ptr = NULL;
	if (asprintf(&ptr, " %-16s (OSS)", name) == -1) return 3;
...
static void _oss_poll(struct midi_provider *_oss_provider)
{
	...
	for (i = 0; i < MAX_OSS_MIDI; i++) {
		...
		if (!_tryopen(i + 1, sbuf, _oss_provider)) continue;
