...
static int sndmgr_wait(schism_audio_device_t *dev)
{
	/* nop */
	return 0;
}
...
static schism_audio_device_t *sndmgr_audio_open_device(uint32_t id, const schism_audio_spec_t *desired, schism_audio_spec_t *obtained)
{
	...
	/* Initialize this BEFORE we go into the callback */
	audio_simple_init(dev, &sndmgr_vtbl, desired->callback, AUDIO_SPEC_SILENCE(*obtained));
