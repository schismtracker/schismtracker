...
#define SCHISM_SIGNED_RSHIFT_VARIANT(type) \
	SCHISM_CONST SCHISM_ALWAYS_INLINE static inline int##type##_t schism_signed_rshift_##type##_( \
		int##type##_t x, unsigned int y) \
	{ \
		return (x < 0) ? ~(~x >> y) : (x >> y); \
	}
...
#define SCHISM_SIGNED_LSHIFT_VARIANT(type) \
	SCHISM_CONST SCHISM_ALWAYS_INLINE static inline int##type##_t schism_signed_lshift_##type##_( \
		int##type##_t x, unsigned int y) \
	{ \
		union { \
			int##type##_t s; \
...
#define lshift_signed(x, y) \
	((sizeof((x) | (y)) <= sizeof(int8_t))           ? (schism_signed_lshift_8_(x, y)) \
		: (sizeof((x) | (y)) <= sizeof(int16_t)) ? (schism_signed_lshift_16_(x, y)) \
		: (sizeof((x) | (y)) <= sizeof(int32_t)) ? (schism_signed_lshift_32_(x, y)) \
		: (sizeof((x) | (y)) <= sizeof(int64_t)) ? (schism_signed_lshift_64_(x, y)) \
...
# define rshift_signed(x, y) \
	 ((sizeof((x) >> (y)) <= sizeof(int8_t))           ? (schism_signed_rshift_8_(x, y)) \
		 : (sizeof((x) >> (y)) <= sizeof(int16_t)) ? (schism_signed_rshift_16_(x, y)) \
		 : (sizeof((x) >> (y)) <= sizeof(int32_t)) ? (schism_signed_rshift_32_(x, y)) \
		 : (sizeof((x) >> (y)) <= sizeof(int64_t)) ? (schism_signed_rshift_64_(x, y)) \
...
#ifndef bswap_64
SCHISM_CONST SCHISM_ALWAYS_INLINE static inline uint64_t bswap_64_schism_internal_(uint64_t x)
{
...
#ifndef bswap_32
SCHISM_CONST SCHISM_ALWAYS_INLINE static inline uint32_t bswap_32_schism_internal_(uint32_t x)
{
...
#ifndef bswap_16
SCHISM_CONST SCHISM_ALWAYS_INLINE static inline uint16_t bswap_16_schism_internal_(uint16_t x)
{
...
#define SCHISM_ABS_VARIANT(BITS) \
	static inline SCHISM_CONST SCHISM_ALWAYS_INLINE uint##BITS##_t babs##BITS(int##BITS##_t x) \
	{ \
...
#define SCHISM_UAVG_VARIANT(BITS) \
	static inline SCHISM_ALWAYS_INLINE SCHISM_CONST uint##BITS##_t bavgu##BITS(uint##BITS##_t x, uint##BITS##_t y) \
	{ \
...
#define SCHISM_SAVG_VARIANT(BITS) \
	static inline SCHISM_ALWAYS_INLINE SCHISM_CONST int##BITS##_t bavgs##BITS(int##BITS##_t x, int##BITS##_t y) \
	{ \
...
static inline SCHISM_ALWAYS_INLINE SCHISM_CONST int bplacesu8(uint8_t x)
{
	if (x < UINT8_C(10))
		return 1;
	if (x < UINT8_C(100))
...
static inline SCHISM_ALWAYS_INLINE SCHISM_CONST int bplacesu16(uint16_t x)
{
	if (x < UINT16_C(10))
		return 1;
	if (x < UINT16_C(100))
...
static inline SCHISM_ALWAYS_INLINE SCHISM_CONST int bplacesu32(uint32_t x)
{
	if (x < UINT32_C(10))
		return 1;
	if (x < UINT32_C(100))
...
static inline SCHISM_ALWAYS_INLINE SCHISM_CONST int bplacesu64(uint64_t x)
{
	if (x < UINT64_C(10))
		return 1;
	if (x < UINT64_C(100))
...
#define SCHISM_SPLACES_VARIANT(BITS) \
	static inline SCHISM_ALWAYS_INLINE SCHISM_CONST int bplacess##BITS(int##BITS##_t x) \
	{ \
...
SCHISM_CONST SCHISM_ALWAYS_INLINE static inline uint32_t bpow32(uint32_t base, uint32_t exponent)
{
	uint32_t r = 1, i;
...
SCHISM_CONST SCHISM_ALWAYS_INLINE static inline uint32_t bsqrt32(uint32_t r)
{
	uint32_t t, b, c = 0;
...
#define BITARRAY_DECLARE(name, size) uint32_t name[(size + 31) / 32]
#define BITARRAY_ZERO(name)          ((void)memset(name, 0, sizeof(name)))

/* this overflows (into bits not defined in size), but I don't
 * think it matters */
#define BITARRAY_FILL(name) ((void)memset(name, 0xFF, sizeof(name)))

/* these should not be touched outside of bits.h.
 * users should use the macros */
static inline SCHISM_ALWAYS_INLINE void barray_set_impl(uint32_t *barray, int bit)
{
	barray[bit >> 5] |= ((uint32_t)1 << (bit & 0x1F));
}

static inline SCHISM_ALWAYS_INLINE void barray_clear_impl(uint32_t *barray, int bit)
{
	barray[bit >> 5] &= ~((uint32_t)1 << (bit & 0x1F));
}

static inline SCHISM_ALWAYS_INLINE SCHISM_PURE int barray_isset_impl(uint32_t *barray, int bit)
{
	return !!(barray[bit >> 5] & ((uint32_t)1 << (bit & 0x1F)));
}

#define BITARRAY_SET(name, bit)   (barray_set_impl(name, bit))
#define BITARRAY_CLEAR(name, bit) (barray_clear_impl(name, bit))
#define BITARRAY_ISSET(name, bit) (barray_isset_impl(name, bit))
...
static inline SCHISM_ALWAYS_INLINE SCHISM_CONST uint32_t bnextpow2(uint32_t x)
{
	x--;
	x |= (x >> 1);
	x |= (x >> 2);
...
static inline SCHISM_ALWAYS_INLINE SCHISM_CONST uint32_t blog2(uint32_t x)
{
...
static inline SCHISM_ALWAYS_INLINE SCHISM_CONST uint32_t breverse32(uint32_t x)
{
...
static inline SCHISM_ALWAYS_INLINE SCHISM_CONST uint32_t bgcd32(uint32_t a, uint32_t b)
{
	while (a != 0) {
		uint32_t tmp = a;
		a = b % tmp;
