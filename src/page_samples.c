...
void sample_set(int n)
{
        ...
        status.flags =
                (status.flags & ~INSTRUMENT_CHANGED) | SAMPLE_CHANGED;
...
static void sample_list_draw_list(void)
{
        ...
        for (pos = 0, n = top_sample; pos < 35; pos++, n++) {
                ...
                draw_text_len(name, 25, 5, 13 + pos, 6,
                              (is_selected ? 14 : 0));
                draw_char(168, 30, 13 + pos, 2, (is_selected ? 14 : 0));
                draw_text("Play", 31, 13 + pos, (has_data ? 6 : 7),
                          (is_selected ? 14 : 0));
        }
        ...
        if (ACTIVE_PAGE.selected_item == 0) {
                ...
                if (sample_list_cursor_pos == 25) {
                        draw_text("Play", 31, 13 + pos, 0,
                                  (has_data ? 3 : 6));
                } else {
                        draw_char((sample_list_cursor_pos >
                                   (signed) strlen(name)
                                   ? 0 : name[sample_list_cursor_pos]),
                                  sample_list_cursor_pos + 5, 13 + pos, 0,
                                  3);
                }
        }
...
static void sample_list_predraw_hook(void)
{
        ...
        items_samplelist[9].menutoggle.state =
                (sample->
                 flags & SAMP_LOOP ? (sample->
                                      flags & SAMP_LOOP_PINGPONG ? 2 : 1) :
                 0);
        /* FIXME: take a closer look at how these work in IT */
        items_samplelist[10].numentry.value = sample->loop_start;
        items_samplelist[10].numentry.max = sample->length;
        items_samplelist[11].numentry.value = sample->loop_end;
        items_samplelist[11].numentry.max = sample->length;
        items_samplelist[12].menutoggle.state =
                (sample->
                 flags & SAMP_SUSLOOP ? (sample->
                                         flags & SAMP_SUSLOOP_PINGPONG ? 2
                                         : 1) : 0);
        items_samplelist[13].numentry.value = sample->sustain_start;
        items_samplelist[13].numentry.max = sample->length;
        items_samplelist[14].numentry.value = sample->sustain_end;
        items_samplelist[14].numentry.max = sample->length;

        items_samplelist[15].togglebutton.state
                = items_samplelist[16].togglebutton.state
                = items_samplelist[17].togglebutton.state =
                items_samplelist[18].togglebutton.state = 0;

        switch (sample->vib_type) {
        case VIB_SINE:
                items_samplelist[15].togglebutton.state = 1;
                break;
        case VIB_RAMP_UP:
        case VIB_RAMP_DOWN:
                items_samplelist[16].togglebutton.state = 1;
                break;
        case VIB_SQUARE:
                items_samplelist[17].togglebutton.state = 1;
                break;
        case VIB_RANDOM:
                items_samplelist[18].togglebutton.state = 1;
                break;
        }
        ...
        draw_text_len((has_data
                       ? (sample->
                          flags & SAMP_16_BIT ? "16 bits" : "8 bits")
                       : "No sample"), 13, 64, 22, 2, 0);
        draw_text_len(numtostr_n(sample->length, buf), 13, 64, 23, 2, 0);
...
static void clear_sample_text(void)
{
        ...
        song_get_sample(current_sample, &name)->filename[0] = 0;
        memset(name, 0, 26);
...
static void do_delete_sample(void)
{
        //void *ptr;
        char *name;
        song_sample *sample = song_get_sample(current_sample, &name);

        sample->length = 0;
        //ptr = sample->data;
        memset(sample, 0, sizeof(*sample));
        name[0] = 0;
        /* doh!! stupid c++ with its stupid 'new' operator...
         * ehehe... FIXME. */
        //free(ptr);
...
static void do_pre_loop_cut(void)
{
        ...
        if (sample->flags & SAMP_16_BIT)
                memmove(sample->data, sample->data + 2 * pos,
                        2 * (sample->length - pos));
        else
                memmove(sample->data, sample->data + pos,
                        sample->length - pos);
        sample->length -= pos;
...
static inline void sample_list_handle_alt_key(SDL_keysym * k)
{
        ...
        case SDLK_a:
                if (sample->data != NULL)
                        dialog_create(DIALOG_OK_CANCEL, "Convert sample?",
                                      do_sign_convert, NULL, 0);
                return;
        case SDLK_b:
                ...
                if (!(sample->data == NULL
                      || (sample->flags & SAMP_SUSLOOP
                          ...
                          && sample->sustain_start == 0)
                      || (sample->loop_start == 0)))
                        dialog_create(DIALOG_OK_CANCEL, "Cut sample?",
                                      do_pre_loop_cut, NULL, 1);
                return;
        case SDLK_d:
                dialog_create(DIALOG_OK_CANCEL, "Delete sample?",
                              do_delete_sample, NULL, 1);
                return;
        case SDLK_g:
                if (sample->data == NULL)
                        return;
        ...
        case SDLK_h:
                if (sample->data != NULL)
                        dialog_create(DIALOG_YES_NO, "Centralise sample?",
                                      do_centralise, NULL, 0);
                return;
        case SDLK_l:
                if (sample->data != NULL
                    && (sample->loop_end != 0 || sample->sustain_end != 0))
                        dialog_create(DIALOG_OK_CANCEL, "Cut sample?",
                                      do_post_loop_cut, NULL, 1);
                return;
        case SDLK_q:
                if (sample->data != NULL)
...
static void update_sample_loop_info(void)
{
        SDL_Rect rect = sample_waveform_area;
        ...
        case 1:        /* forwards */
                ...
                sample->flags &= ~SAMP_LOOP_PINGPONG;
                break;
        case 2:        /* ping pong */
                sample->flags |= (SAMP_LOOP | SAMP_LOOP_PINGPONG);
                break;
        default:       /* hopefully 0 (off) */
                sample->flags &= ~(SAMP_LOOP | SAMP_LOOP_PINGPONG);
                break;
        }
        ...
        case 1:
                sample->flags |= SAMP_SUSLOOP;
                sample->flags &= ~SAMP_SUSLOOP_PINGPONG;
                break;
        case 2:
                sample->flags |= (SAMP_SUSLOOP | SAMP_SUSLOOP_PINGPONG);
                break;
        default:
                sample->flags &= ~(SAMP_SUSLOOP | SAMP_SUSLOOP_PINGPONG);
                break;
...
void sample_list_load_page(struct page *page)
{
        ...
        /* 0 = sample list */
        items_samplelist[0].type = ITEM_OTHER;
        items_samplelist[0].next.tab = 1;
        items_samplelist[0].other.handle_key =
                sample_list_handle_key_on_list;
        items_samplelist[0].other.redraw = sample_list_draw_list;
        /* 1 -> 6 = middle column */
        create_thumbbar(items_samplelist + 1, 38, 16, 9, 1, 2, 7,
                        update_values_in_song, 0, 64);
        create_thumbbar(items_samplelist + 2, 38, 23, 9, 1, 3, 7,
                        update_values_in_song, 0, 64);
        create_toggle(items_samplelist + 3, 38, 30, 2, 4, 0, 7, 7,
                      update_values_in_song);
        create_thumbbar(items_samplelist + 4, 38, 31, 9, 3, 5, 7,
                        update_values_in_song, 0, 64);
        create_thumbbar(items_samplelist + 5, 38, 39, 9, 4, 6, 15,
                        update_values_in_song, 0, 64);
        create_thumbbar(items_samplelist + 6, 38, 46, 9, 5, 6, 19,
                        update_values_in_song, 0, 32);
        /* 7 -> 14 = top right box */
        create_textentry(items_samplelist + 7, 64, 13, 13, 7, 8, 0, NULL,
                         NULL, NULL, 12);
        create_numentry(items_samplelist + 8, 64, 14, 7, 7, 9, 0,
                        update_values_in_song, 0, 9999999,
                        &sample_numentry_cursor_pos);
        create_menutoggle(items_samplelist + 9, 64, 15, 8, 10, 1, 0, 0,
                          update_sample_loop_info, loop_states);
        create_numentry(items_samplelist + 10, 64, 16, 7, 9, 11, 0,
                        update_sample_loop_info, 0, 0,
                        &sample_numentry_cursor_pos);
        ...
        create_togglebutton(items_samplelist + 15, 57, 36, 6, 14, 17, 5,
                            16, 16, update_values_in_song, "\xb9\xba", 3,
                            vibrato_waveforms);
        create_togglebutton(items_samplelist + 16, 67, 36, 6, 14, 18, 15,
                            0, 0, update_values_in_song, "\xbd\xbe", 3,
                            vibrato_waveforms);
        create_togglebutton(items_samplelist + 17, 57, 39, 6, 15, 19, 5,
                            18, 18, update_values_in_song, "\xbb\xbc", 3,
                            vibrato_waveforms);
        create_togglebutton(items_samplelist + 18, 67, 39, 6, 16, 19, 17,
                            0, 0, update_values_in_song, "Random", 1,
                            vibrato_waveforms);
        /* 19 = vibrato rate */
        create_thumbbar(items_samplelist + 19, 56, 46, 16, 17, 19, 0,
                        update_values_in_song, 0, 255);
}
