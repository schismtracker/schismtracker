...
static void song_vars_draw_const(void)
{
        ...
        draw_text_unlocked("Separation", 6, 25, 1 /* greyed out */ , 2);
        ...
        /* this is the pan separation */
        draw_text_unlocked("                \x9c", 17, 25, 1, 0);
        draw_text_unlocked("128", 35, 25, 1, 2);
...
static void song_changed_cb(void)
{
        ...
        if (song_is_instrument_mode()) {
                items_vars[8].togglebutton.state = 1;
                items_vars[9].togglebutton.state = 0;
        } else {
                items_vars[8].togglebutton.state = 0;
                items_vars[9].togglebutton.state = 1;
        }
        if (song_is_stereo()) {
                items_vars[10].togglebutton.state = 1;
                items_vars[11].togglebutton.state = 0;
        } else {
                items_vars[10].togglebutton.state = 0;
                items_vars[11].togglebutton.state = 1;
        }
        if (song_has_linear_pitch_slides()) {
                items_vars[12].togglebutton.state = 1;
                items_vars[13].togglebutton.state = 0;
        } else {
                items_vars[12].togglebutton.state = 0;
                items_vars[13].togglebutton.state = 1;
        }
...
/* --------------------------------------------------------------------- */
/* lousy hack. separation is currently unimplemented, but i don't want to
 * have to fix all the numbers when i get it working. */

static void nop_redraw(void)
{
}
...
void song_vars_load_page(struct page *page)
{
        ...
        create_textentry(items_vars, 17, 16, 26, 0, 1, 1,
                         update_song_title, NULL, song_get_title(), 25);
        /* 1 = tempo */
        ...
        /* 4 = mixing volume */
        create_thumbbar(items_vars + 4, 17, 24, 17, 3, 6, 6,
                        update_values_in_song, 0, 128);
        /* 5 = separation (unimplemented) */
        items_vars[5].type = ITEM_OTHER;
        items_vars[5].other.redraw = nop_redraw;

        /* 6 = old effects */
        create_toggle(items_vars + 6, 17, 26, 4, 7, 4, 7, 7,
