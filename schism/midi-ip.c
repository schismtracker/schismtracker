...
static void _readin(struct midi_provider *p, int en, int fd)
{
	...
	r = recvfrom(fd, (char *)buffer, sizeof(buffer), 0, (struct sockaddr *)&asin, &slen);
	if (r > 0) {
		ptr = src = NULL;
...
static int _ip_start(struct midi_port *p)
{
	...
	SDL_LockMutex(blocker);
	if (p->io & MIDI_INPUT) state[n] |= 1;
	if (p->io & MIDI_OUTPUT) state[n] |= 2;
	SDL_UnlockMutex(blocker);
	do_wake_midi();
	return 1;
}
...
static void _ip_send(struct midi_port *p, const unsigned char *data, unsigned int len, UNUSED unsigned int delay)
{
	struct sockaddr_in asin = {0};
	unsigned char *ipcopy;
	int n = INT_SHAPED_PTR(p->userdata);
	...
	while (len) {
		...
		if (sendto(out_fd, (const char *)data, ss, 0, (struct sockaddr *)&asin, sizeof(asin)) < 0) {
...
static void _ip_poll(struct midi_provider *p)
{
	...
	if (m < last_buildout) {
		...
		for (i = last_buildout; i < m; i++) {
			...
			midi_port_register(p, MIDI_INPUT | MIDI_OUTPUT, buffer, PTR_SHAPED_INT((intptr_t)i), 0);
		}
		last_buildout = m;
	}
	SDL_UnlockMutex(blocker);
