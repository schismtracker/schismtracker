...
{
	if (!NO_CAM_MODS(k->mod))
		return -1;
	switch (k->sym) {
#define QZA(n) \
	case SCHISM_KEYSYM_##n: \
		return get_effect_number(#n[0])
		QZA(a);
		QZA(b);
		QZA(c);
		QZA(d);
...
void kbd_key_translate(struct key_event *k)
{
	...
	if (k->text) {
		 ...
		 * for some reason */
		case '/':
			k->sym = SCHISM_KEYSYM_SLASH;
			break;
		case '\\':
			k->sym = SCHISM_KEYSYM_BACKSLASH;
			break;
	...
	if (k->mod & SCHISM_KEYMOD_GUI) {
		k->mod = ((k->mod & ~SCHISM_KEYMOD_GUI)
			  | ((status.flags & META_IS_CTRL) ? SCHISM_KEYMOD_CTRL : SCHISM_KEYMOD_ALT));
	}
	...
	if (k->mod & SCHISM_KEYMOD_NUM) {
		...
		case SCHISM_KEYSYM_KP_0:
			k->sym = SCHISM_KEYSYM_0;
			k->mod &= ~SCHISM_KEYMOD_NUM;
			break;
		case SCHISM_KEYSYM_KP_1:
			k->sym = SCHISM_KEYSYM_1;
			k->mod &= ~SCHISM_KEYMOD_NUM;
			break;
		case SCHISM_KEYSYM_KP_2:
			k->sym = SCHISM_KEYSYM_2;
			k->mod &= ~SCHISM_KEYMOD_NUM;
			break;
		case SCHISM_KEYSYM_KP_3:
			k->sym = SCHISM_KEYSYM_3;
			k->mod &= ~SCHISM_KEYMOD_NUM;
			break;
		...
		case SCHISM_KEYSYM_KP_5:
			k->sym = SCHISM_KEYSYM_5;
			k->mod &= ~SCHISM_KEYMOD_NUM;
			break;
		case SCHISM_KEYSYM_KP_6:
			k->sym = SCHISM_KEYSYM_6;
			k->mod &= ~SCHISM_KEYMOD_NUM;
			break;
		case SCHISM_KEYSYM_KP_7:
			k->sym = SCHISM_KEYSYM_7;
			k->mod &= ~SCHISM_KEYMOD_NUM;
			break;
		case SCHISM_KEYSYM_KP_8:
			k->sym = SCHISM_KEYSYM_8;
			k->mod &= ~SCHISM_KEYMOD_NUM;
			break;
		case SCHISM_KEYSYM_KP_9:
			k->sym = SCHISM_KEYSYM_9;
			k->mod &= ~SCHISM_KEYMOD_NUM;
			break;
		case SCHISM_KEYSYM_KP_PERIOD:
			k->sym = SCHISM_KEYSYM_PERIOD;
			k->mod &= ~SCHISM_KEYMOD_NUM;
			break;
		...
		case SCHISM_KEYSYM_KP_ENTER:
			k->sym = SCHISM_KEYSYM_RETURN;
			k->mod &= ~SCHISM_KEYMOD_NUM;
			break;
		case SCHISM_KEYSYM_KP_EQUALS:
			k->sym = SCHISM_KEYSYM_EQUALS;
			k->mod &= ~SCHISM_KEYMOD_NUM;
			break;
		default:
			break;
		...
		case SCHISM_KEYSYM_KP_0:
			...
			break;
		case SCHISM_KEYSYM_KP_4:
			k->sym = SCHISM_KEYSYM_LEFT;
			break;
		case SCHISM_KEYSYM_KP_6:
			k->sym = SCHISM_KEYSYM_RIGHT;
			break;
		case SCHISM_KEYSYM_KP_2:
			k->sym = SCHISM_KEYSYM_DOWN;
			break;
		case SCHISM_KEYSYM_KP_8:
			k->sym = SCHISM_KEYSYM_UP;
			break;

		case SCHISM_KEYSYM_KP_9:
			k->sym = SCHISM_KEYSYM_PAGEUP;
			break;
		case SCHISM_KEYSYM_KP_3:
			k->sym = SCHISM_KEYSYM_PAGEDOWN;
			break;

		case SCHISM_KEYSYM_KP_7:
			k->sym = SCHISM_KEYSYM_HOME;
			break;
		case SCHISM_KEYSYM_KP_1:
			k->sym = SCHISM_KEYSYM_END;
			break;
...
int numeric_key_event(struct key_event *k, int kponly)
{
	...
	case SCHISM_KEYSYM_0:
	case SCHISM_KEYSYM_KP_0:
		return 0;
	case SCHISM_KEYSYM_1:
	case SCHISM_KEYSYM_KP_1:
		return 1;
	case SCHISM_KEYSYM_2:
	case SCHISM_KEYSYM_KP_2:
		return 2;
	case SCHISM_KEYSYM_3:
	case SCHISM_KEYSYM_KP_3:
		return 3;
	case SCHISM_KEYSYM_4:
	case SCHISM_KEYSYM_KP_4:
		return 4;
	case SCHISM_KEYSYM_5:
	case SCHISM_KEYSYM_KP_5:
		return 5;
	case SCHISM_KEYSYM_6:
	case SCHISM_KEYSYM_KP_6:
		return 6;
	case SCHISM_KEYSYM_7:
	case SCHISM_KEYSYM_KP_7:
		return 7;
	case SCHISM_KEYSYM_8:
	case SCHISM_KEYSYM_KP_8:
		return 8;
	case SCHISM_KEYSYM_9:
	case SCHISM_KEYSYM_KP_9:
		return 9;
	default:
		break;
...
int kbd_get_note(struct key_event *k)
{
	...
	if (k->mod & SCHISM_KEYMOD_SHIFT) {
		...
		case SCHISM_SCANCODE_R:
			note = 18;
			break;
		case SCHISM_SCANCODE_5:
			note = 19;
			break;
		case SCHISM_SCANCODE_T:
			note = 20;
			break;
		case SCHISM_SCANCODE_6:
			note = 21;
			break;
		...
		case SCHISM_SCANCODE_I:
			note = 25;
			break;
		case SCHISM_SCANCODE_9:
			note = 26;
			break;
		case SCHISM_SCANCODE_O:
			note = 27;
			break;
		case SCHISM_SCANCODE_0:
			note = 28;
			break;
...
int kbd_get_alnum(struct key_event *k)
{
	...
	if (k->mod & SCHISM_KEYMOD_SHIFT) {
		...
		default:
			return k->sym; // shift + some weird key = ???
		}
	}
