...
void create_other(
	struct widget *w, int next_tab, int (*i_handle_key)(struct key_event *k),
...
static void textentry_reposition(struct widget *w)
{
	...
	if (w->d.textentry.cursor_pos < w->d.textentry.firstchar) {
		...
		if (w->d.textentry.firstchar < 0) w->d.textentry.firstchar = 0;
	}
}
...
int textentry_add_text(struct widget *w, const uint8_t *text)
{
	...
	for (; *text; text++)
		if (!textentry_add_char(w, *text)) return 0;
...
{
	if (((k->mod & (KMOD_CTRL | KMOD_ALT | KMOD_GUI)) == 0) && w->d.menutoggle.activation_keys) {
...
{
	if (((k->mod & (KMOD_CTRL | KMOD_ALT | KMOD_GUI)) == 0) && w->d.bitset.activation_keys) {
...
void togglebutton_set(struct widget *p_widgets, int widget, int do_callback)
{
	...
	for (i = 0; group[i] >= 0; i++) p_widgets[group[i]].d.togglebutton.state = 0;
	p_widgets[widget].d.togglebutton.state = 1;

	if (do_callback && p_widgets[widget].changed) p_widgets[widget].changed();
