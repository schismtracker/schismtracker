...
struct it_palette palettes[] = {
	...
	{"Gold (Vintage)",
     {
		 /* more directly based on the ST3 palette */
		 /*  0 */ {0, 0, 0},
		 /*  1 */ {20, 17, 10},
		 /*  2 */ {41, 36, 21},
		 /*  3 */ {63, 55, 33},
	...
	{"Atlantic",
     {
		 /* from an ANCIENT version of Impulse Tracker */
		 /*  0 */ {0, 0, 0},
		 /*  1 */ {2, 0, 30},
		 /*  2 */ {8, 18, 43},
		 /*  3 */ {21, 43, 63},
...
{
	if (palette_index < 0 || palette_index >= NUM_PALETTES) return;
...
{
	for (int n = 0; n < 48; n++) str_out[n] = palette_trans[palettes[which].colors[n / 3][n % 3]];
...
int set_palette_from_string(const char *str_in)
{
	...
	for (int n = 0; n < 48; n++) {
		if (str_in[n] == '\0' || (ptr = strchr(palette_trans, str_in[n])) == NULL) return 0;
