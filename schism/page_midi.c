...
static void midi_page_draw_portlist(void)
{
	...
	for (i = 0; i < 13; i++) {
		...
		if (status.flags & MIDI_EVENT_CHANGED && difftime(now, status.last_midi_time) < 3.0
...
void midi_load_page(struct page *page)
{
	...
	create_button(widgets_midi + 13, 2, 41, 27, 6, 14, 12, 12, 12, midi_output_config, "MIDI Output Configuration", 2);
	create_button(widgets_midi + 14, 2, 44, 27, 13, 14, 12, 12, 12, cfg_midipage_save, "Save Output Configuration", 2);
}
