...
static void order_pan_vol_handle_key(struct key_event *k)
{
	...
	n = CLAMP(n, 1, 64);
	if (ACTIVE_PAGE.selected_widget != n) change_focus_to(n);
}
...
void orderpan_load_page(struct page *page)
{
	...
	for (n = 2; n <= 32; n++) {
		create_panbar(widgets_orderpan + n, 20, 14 + n, n - 1, n + 1, n + 32, orderpan_update_values_in_song, n);
		create_panbar(widgets_orderpan + n + 31, 54, 13 + n, n + 30, n + 32, 0, orderpan_update_values_in_song, n + 31);
	}
