...
void vgamem_ovl_apply(struct vgamem_overlay *n)
{
	...
	for (y = n->y1; y <= n->y2; y++)
		for (x = n->x1; x <= n->x2; x++) vgamem[x + (y * 80)].font = VGAMEM_FONT_OVERLAY;
}
...
#define VGAMEM_SCANNER_VARIANT(BITS) \
 void vgamem_scan##BITS( \
	 uint32_t ry, uint##BITS##_t *out, uint32_t tc[16], uint32_t mouseline[80], uint32_t mouseline_mask[80]) \
 { \
  ...
  for (x = 0; x < 80; x++, bp++, q += 8) { \
   ...
   case VGAMEM_FONT_BIOS: \
	...
	if (bp->font == VGAMEM_FONT_BIOS) { \
	 dg = (bp->character.cp437.c & 0x80) ? bios[(bp->character.cp437.c & 0x7F) << 3] : \
...
int draw_text_utf8(const char *text, int x, int y, uint32_t fg, uint32_t bg)
{
	...
	if (!composed) return draw_text_bios(text, x, y, fg, bg);
	...
	int n;
	for (n = 0; decoder.state == DECODER_STATE_NEED_MORE && !charset_decode_next(&decoder, CHARSET_UTF8)
...
int draw_text_utf8_len(const char *text, int len, int x, int y, uint32_t fg, uint32_t bg)
{
	...
	int n;
	for (n = 0; n < len && decoder.state == DECODER_STATE_NEED_MORE && !charset_decode_next(&decoder, CHARSET_UTF8)
	            && decoder.state != DECODER_STATE_DONE;
	     n++)
		draw_char_unicode(decoder.codepoint, x + n, y, fg, bg);
...
static inline void _draw_thumb_bar_internal(int width, int x, int y, int val, uint32_t fg)
{
	...
	if (++n < width) draw_char(thumb_chars[1][val], x + n, y, fg, 0);
	if (++n < width) draw_fill_chars(x + n, y, x + width - 1, y, DEFAULT_FG, 0);
}
...
void draw_thumb_bar(int x, int y, int width, int min, int max, int val, int selected)
{
	...
	/* draw the bar */
	if (!max) _draw_thumb_bar_internal(width, x, y, 0, selected ? 3 : 2);
	else _draw_thumb_bar_internal(width, x, y, val * (width - 1) * 8 / max, selected ? 3 : 2);
}
...
void draw_vu_meter(int x, int y, int width, int val, int color, int peak)
{
	...
	leftover = val & 7;
	val >>= 3;
	if ((val < chunks - 1) || (status.flags & CLASSIC_MODE)) peak = color;
...
#define DRAW_SAMPLE_DATA_VARIANT(bits) \
 static void _draw_sample_data_##bits( \
	 struct vgamem_overlay *r, int##bits##_t *data, unsigned long length, unsigned int inputchans, \
...
static void _draw_sample_loop(struct vgamem_overlay *r, song_sample_t *sample)
{
	...
	if (!(sample->flags & CHN_LOOP)) return;
...
static void _draw_sample_susloop(struct vgamem_overlay *r, song_sample_t *sample)
{
	...
	if (!(sample->flags & CHN_SUSTAINLOOP)) return;
...
static void _draw_sample_play_marks(struct vgamem_overlay *r, song_sample_t *sample)
{
	...
	if (song_get_mode() == MODE_STOPPED) return;
	...
	while (n--) {
		...
		if (channel->current_sample_data != sample->data) continue;
...
void draw_sample_data(struct vgamem_overlay *r, song_sample_t *sample)
{
	...
	if (sample->flags & CHN_ADLIB) {
		...
		sprintf(
			buf1, "%02X %02X %02X %02X %02X %02X", // length:6*3-1=17
		...
		sprintf(
			buf2, "%02X %02X %02X %02X %02X", // length: 5*3-1=14
	...
	else _draw_sample_data_8(r, sample->data, sample->length * chans, chans, chans);

	if ((status.flags & CLASSIC_MODE) == 0) _draw_sample_play_marks(r, sample);
