...
#define VGA_CHAR_COUNT (VGA_CHAR_WIDTH * VGA_CHAR_HEIGHT)

static uint32_t vgamem[VGA_CHAR_COUNT] = {0};
static uint32_t vgamem_read[VGA_CHAR_COUNT] = {0};
...
void vgamem_ovl_apply(struct vgamem_overlay *n)
{
	...
	for (y = n->y1; y <= n->y2; y++)
		for (x = n->x1; x <= n->x2; x++)
			vgamem[x + (y*VGA_CHAR_WIDTH)] = VGAMEM_FONT_OVERLAY;
}
...
#define VGAMEM_SCANNER_VARIANT(BITS) \
	void vgamem_scan##BITS(uint32_t ry, uint##BITS##_t *out, uint32_t tc[16],\
		uint32_t mouseline[VGA_CHAR_WIDTH], uint32_t mouseline_mask[VGA_CHAR_WIDTH]) \
	{ \
		/* constants */ \
		...
		const uint32_t *bp = &vgamem_read[y * VGA_CHAR_WIDTH]; \
	\
		uint_fast32_t x; \
		for (x = 0; x < VGA_CHAR_WIDTH; x++, bp++, q += 8) { \
...
{
	SCHISM_RUNTIME_ASSERT(x >= 0 && y >= 0 && x < VGA_CHAR_WIDTH && y < VGA_CHAR_HEIGHT, "Coordinates should always be inbounds");

	vgamem[x + (y*VGA_CHAR_WIDTH)] = (VGAMEM_FONT_UNICODE
...
void draw_char_bios(uint8_t c, int x, int y, uint32_t fg, uint32_t bg)
{
	...
	vgamem[x + (y*VGA_CHAR_WIDTH)] = (VGAMEM_FONT_BIOS
		| (fg << VGAMEM_FG_BIT)
		| (bg << VGAMEM_BG_BIT)
		| c);
}
...
void draw_char(uint8_t c, int x, int y, uint32_t fg, uint32_t bg)
{
	...
	vgamem[x + (y*VGA_CHAR_WIDTH)] = ((fg << VGAMEM_FG_BIT) | (bg << VGAMEM_BG_BIT) | c);
}
...
void draw_fill_chars(int xs, int ys, int xe, int ye, uint32_t fg, uint32_t bg)
{
	...
	mm = &vgamem[(ys * VGA_CHAR_WIDTH) + xs];
	len = (xe - xs)+1;
	ye -= ys;
	do {
		for (x = 0; x < len; x++)
			   ...
			   uint32_t fg1, uint32_t bg1, uint32_t fg2, uint32_t bg2)
{
	...
	vgamem[x + (y*VGA_CHAR_WIDTH)] = (VGAMEM_FONT_HALFWIDTH
