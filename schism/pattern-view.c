...
#define MASK_CHAR(field, pos, pos2) \
 mask_chars \
	 [((cursor_pos == 0) ? 1 : 0) | ((cursor_pos == pos) ? 2 : 0) | ((pos2 && cursor_pos == pos2) ? 2 : 0) \
	  | ((mask & field) ? 4 : 0)]
...
void draw_note_13(int x, int y, const song_note_t *note, int cursor_pos, int fg, int bg)
{
	...
	if (note->instrument) num99tostr(note->instrument, instbuf);
	else strcpy(instbuf, "\xad\xad");

	snprintf(note_text, 16, "%s %s %s %c%02X", note_buf, instbuf, vol_buf, get_effect_char(note->effect), note->param);

	if (show_default_volumes && note->voleffect == VOLFX_NONE && note->instrument > 0 && NOTE_IS_NOTE(note->note)) {
		song_sample_t *smp =
			song_is_instrument_mode() ?
				csf_translate_keyboard(current_song, song_get_instrument(note->instrument), note->note, NULL) :
	...
	if (note->voleffect == VOLFX_PANNING) draw_text(vol_buf, x + 7, y, 2, bg);
...
void draw_note_10(int x, int y, const song_note_t *note, int cursor_pos, UNUSED int fg, int bg)
{
	...
	get_volume_string(note->volparam, note->voleffect, vol_buf);
	sprintf(effect_buf, "%c%02X", get_effect_char(note->effect), note->param);
...
void draw_note_7(int x, int y, const song_note_t *note, int cursor_pos, UNUSED int fg, int bg)
{
	...
	get_note_string(note->note, note_buf);
	if (note->instrument) num99tostr(note->instrument, ins_buf);
	else ins_buf[0] = ins_buf[1] = 173;
	get_volume_string(note->volparam, note->voleffect, vol_buf);
...
{
	char buf[] = {
		MASK_CHAR(MASK_NOTE, 0, 0),   MASK_CHAR(MASK_NOTE, 0, 0),
		MASK_CHAR(MASK_NOTE, 0, 1),   MASK_CHAR(MASK_INSTRUMENT, 2, 3),
		MASK_CHAR(MASK_VOLUME, 4, 5), MASK_CHAR(MASK_EFFECT, 6, 0),
		MASK_CHAR(MASK_EFFECT, 7, 8), 0,
	};
...
void draw_note_1(int x, int y, const song_note_t *note, int cursor_pos, int fg, int bg)
{
	...
	if (note->note) {
		...
		if (cursor_pos != 0) fg = (note->voleffect == VOLFX_PANNING) ? 1 : 2;
		get_volume_string(note->volparam, note->voleffect, buf);
		draw_half_width_chars(buf[0], buf[1], x, y, fg, bg, fg, bg);
	} else if (note->effect || note->param) {
...
void draw_note_6(int x, int y, const song_note_t *note, int cursor_pos, UNUSED int fg, int bg)
{
	...
	if (cursor_pos == 0) draw_char(note_buf[0], x, y, 0, 3);
	else draw_char(note_buf[0], x, y, fg, bg);
