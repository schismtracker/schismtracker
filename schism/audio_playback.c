...
static void audio_callback(UNUSED void *qq, uint8_t *stream, int len)
{
	...
	} else if (status.current_page == PAGE_WATERFALL || status.vis_style == VIS_FFT) {
		if (audio_output_channels == 2) {
	...
	if (current_song->num_voices > max_channels_used) max_channels_used = MIN(current_song->num_voices, max_voices);
POST_EVENT:
...
{
	for (int count = 0; count < audio_device_list_size; count++) free(audio_device_list[count].name);
...
static void main_song_mode_changed_cb(void)
{
	...
	for (n = 0; n < PAGE_MAX; n++) {
		if (pages[n].song_mode_changed_cb) pages[n].song_mode_changed_cb();
	}
}
...
void song_change_current_play_channel(int relative, int wraparound)
{
	...
	if (wraparound) {
		if (current_play_channel < 1) current_play_channel = 64;
		else if (current_play_channel > 64) current_play_channel = 1;
	} else {
...
static int song_keydown_ex(int samp, int ins, int note, int vol, int chan, int effect, int param)
{
	...
	if (NOTE_IS_NOTE(note)) {
		...
		if (samp == 0) samp = c->last_instrument;
		else if (samp == KEYJAZZ_INST_FAKE) samp = 0; // dumb hack
		if (ins == 0) ins = c->last_instrument;
		else if (ins == KEYJAZZ_INST_FAKE) ins = 0; // dumb hack
	...
	if (i) csf_check_nna(current_song, chan_internal, ins, note, 0);
	if (s) {
		...
		if (i) {
			...
			if (i->ifc & 0x80) c->cutoff = i->ifc & 0x7f;
			if (i->ifr & 0x80) c->resonance = i->ifr & 0x7f;
			//?
		...
		if (i) c->instrument_volume = (c->instrument_volume * i->global_volume) >> 7;
		c->global_volume = 64;
	...
	if (c->increment < 0) c->increment = -c->increment; // lousy hack
...
void song_single_step(int patno, int row)
{
	...
	for (i = 1; i <= 64; i++, cx++, cur_note++) {
		...
		if (song_is_instrument_mode()) {
			if (ins < 1) ins = KEYJAZZ_NOINST;
			smp = -1;
		} else {
			if (smp < 1) smp = KEYJAZZ_NOINST;
			ins = -1;
		}
...
void song_pause(void)
{
	...
	if (!(current_song->flags & SONG_PAUSED)) current_song->flags ^= SONG_ENDREACHED;
	song_unlock_audio();
	main_song_mode_changed_cb();
}
			...
			if (note_tracker[chan] != 0) {
				for (int j = 0; j < 16; j++) {
					csf_process_midi_macro(
						current_song, chan, current_song->midi_config.note_off, 0, note_tracker[chan], 0, j);
				}
				moff[0] = 0x80 + chan;
...
{
	if ((current_song->flags & (SONG_ENDREACHED | SONG_PAUSED)) == (SONG_ENDREACHED | SONG_PAUSED)) return MODE_STOPPED;
	if (current_song->flags & SONG_PAUSED) return MODE_SINGLE_STEP;
	if (current_song->flags & SONG_PATTERNPLAYBACK) return MODE_PATTERN_LOOP;
	return MODE_PLAYING;
}
...
void song_update_playing_sample(int s_changed)
{
	...
	while (n--) {
		...
		if (channel->ptr_sample && channel->current_sample_data) {
			...
			if (inst->flags
			    & (CHN_PINGPONGSUSTAIN | CHN_SUSTAINLOOP | CHN_PINGPONGFLAG | CHN_PINGPONGLOOP | CHN_LOOP)) {
			...
			channel->flags &= ~(CHN_PINGPONGSUSTAIN | CHN_PINGPONGLOOP | CHN_PINGPONGFLAG | CHN_SUSTAINLOOP | CHN_LOOP);
			channel->flags |=
				inst->flags & (CHN_PINGPONGSUSTAIN | CHN_PINGPONGLOOP | CHN_PINGPONGFLAG | CHN_SUSTAINLOOP | CHN_LOOP);
...
{
	if (on) current_song->mix_flags &= ~SNDMIX_NOSURROUND;
	else current_song->mix_flags |= SNDMIX_NOSURROUND;
...
void cfg_load_audio(cfg_file_t *cfg)
{
	...
	if (audio_settings.channels != 1 && audio_settings.channels != 2) audio_settings.channels = 2;
	if (audio_settings.bits != 8 && audio_settings.bits != 16) audio_settings.bits = 16;
...
static void _schism_midi_out_note(int chan, const song_note_t *starting_note)
{
	...
	if (ins < 0 || ins >= MAX_INSTRUMENTS) return; /* err...  almost certainly */
	...
	if (current_song->instruments[ins]->midi_channel_mask >= 0x10000) {
		...
		if (current_song->instruments[ins]->midi_channel_mask > 0)
			while (!(current_song->instruments[ins]->midi_channel_mask & (1 << mc))) ++mc;
	}
...
printf("channel = %d note=%d starting_note=%p\n",chan,m_note,starting_note);
	...
	if (m_note > 120) {
		if (note_tracker[chan] != 0) {
			csf_process_midi_macro(
				current_song, chan, current_song->midi_config.note_off, 0, note_tracker[chan], 0, ins_tracker[chan]);
		}
	...
	mg = (current_song->instruments[ins]->midi_program) + ((midi_flags & MIDI_BASE_PROGRAM1) ? 1 : 0);
	...
	if (mg > -1 && was_program[mc] != mg) {
		was_program[mc] = mg;
		csf_process_midi_macro(
			current_song, chan, current_song->midi_config.set_program, mg, 0, 0, ins); // program change
	}
	if (c->flags & CHN_MUTE) {
		...
		csf_process_midi_macro(
			current_song, chan, current_song->midi_config.note_on, 0, need_note, need_velocity, ins); // noteon
	} else if (need_velocity > -1 && note_tracker[chan] > 0) {
		need_velocity = CLAMP(need_velocity * 2, 0, 127);
		csf_process_midi_macro(
			current_song, chan, current_song->midi_config.set_volume, need_velocity, note_tracker[chan], need_velocity,
			ins); // volume-set
	}
}
...
static SDLCALL int schism_init_audio_impl(const char *name)
{
	...
	if (name) SDL_setenv("SDL_AUDIODRIVER", name, 1);
...
static int _audio_open_driver(const char *driver)
{
	...
	SDL_version ver;
	SDL_GetVersion(&ver);
	if ((ver.major >= 2) && (ver.major > 2 || ver.minor >= 0) && (ver.major > 2 || ver.minor > 0 || ver.patch >= 18)) {
	...
	if (driver && *driver) {
		/* compatibility! */
		n = !strcmp(driver, "oss") ? "dsp" : (!strcmp(driver, "nosound") || !strcmp(driver, "none")) ? "dummy" : driver;

		if (!sdl_audio_init(n)) goto audio_was_init;
	}
...
static int _audio_open_device(const char *device, int verbose)
{
	...
	int size_pow2 = 2;
	while (size_pow2 < audio_settings.buffer_size) size_pow2 <<= 1;
...
success:
	...
	csf_set_wave_config(current_song, obtained.freq, SDL_AUDIO_BITSIZE(obtained.format), obtained.channels);
	...
	if (verbose) {
		...
		log_appendf(
			5, " %d Hz, %d bit, %s", obtained.freq, SDL_AUDIO_BITSIZE(obtained.format),
...
static int _audio_init_head(const char *driver, const char *device, int verbose)
{
	...
	if (!driver || !*driver) driver = cfg_audio_driver;

	if (!device || !*device) device = cfg_audio_device;
...
void audio_flash_reinitialized_text(int success)
{
	if (success) {
		status_text_flash(
			(status.flags & CLASSIC_MODE) ? "Sound Blaster 16 reinitialised" : "Audio output reinitialised");
	} else {
		/* ... */
...
int audio_init(const char *driver, const char *device)
{
	...
	if (status.flags & CLASSIC_MODE) song_stop();
...
void song_init_modplug(void)
{
	...
	if (audio_settings.no_ramping) current_song->mix_flags |= SNDMIX_NORAMPING;
	else current_song->mix_flags &= ~SNDMIX_NORAMPING;
