...
static int _audio_open_driver(const char *driver)
{
	...
	if (driver && *driver) {
		...
		if (!SDL_AudioInit(n)) goto audio_was_init;
	}
...
fail:
	...
	if (!_audio_open_driver("dummy") || !_audio_open_device(NULL, verbose)) goto catastrophy;
...
void audio_reinit(const char *device)
{
	...
	else status_text_flash("Audio output reinitialised");
}
