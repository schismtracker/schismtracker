...
static void resize_sample_dialog(int aa)
{
	...
	create_button(resize_sample_widgets + 1, 36, 30, 6, 0, 1, 1, 1, 1, dialog_cancel_NULL, "Cancel", 1);
...
void sample_list_load_page(struct page *page)
{
	...
	create_numentry(
		...
		&sample_numentry_cursor_pos);
	create_menutoggle(widgets_samplelist + 12, 64, 18, 11, 13, 1, 0, 0, update_sample_loop_flags, loop_states);
	create_numentry(
