...
static void preferences_draw_const(void)
{
	...
	draw_box(AUDIO_DEVICE_BOX_X - 1, AUDIO_DEVICE_BOX_Y - 1, AUDIO_DEVICE_BOX_END_X + 1, AUDIO_DEVICE_BOX_END_Y + 1,
		BOX_THICK | BOX_INNER | BOX_INSET);

	draw_text("Available Audio Drivers", AUDIO_DRIVER_BOX_X, AUDIO_DRIVER_BOX_Y - 2, 0, 2);
	draw_box(AUDIO_DRIVER_BOX_X - 1, AUDIO_DRIVER_BOX_Y - 1, AUDIO_DRIVER_BOX_END_X + 1, AUDIO_DRIVER_BOX_END_Y + 1,
		BOX_THICK | BOX_INNER | BOX_INSET);
...
static void audio_device_list_activate_(void)
{
	...
	uint32_t id = !w->d.listbox.focus ? AUDIO_BACKEND_DEFAULT : audio_device_list[w->d.listbox.focus - 1].id;
...
void preferences_load_page(struct page *page)
{
	...
	widgets_preferences[0].next.left = widgets_preferences[0].next.right = widgets_preferences[0].next.tab
		= widgets_preferences[0].next.backtab = widgets_preferences[1].next.left
		= widgets_preferences[1].next.right = widgets_preferences[1].next.tab
		= widgets_preferences[1].next.backtab = interp_modes + 13;

	for (i = 0; interpolation_modes[i]; i++) {
		snprintf(buf, sizeof(buf), "%d Bit, %s", audio_settings.bits, interpolation_modes[i]);
		ptr = str_dup(buf);
		widget_create_togglebutton(widgets_preferences + i + 2, 6, 20 + (i * 3), 26, i + 1, i + 3, i + 2,
			interp_modes + 11, i + 3, change_mixer, ptr, 2, interp_group);
		if (i < 1)
	...
	for (j = 0; j < 4; j++) {
		...
		if (j == 0)
			n = i + 1;
		widget_create_thumbbar(widgets_preferences + i + 2 + (j * 2), 26, 23 + (i * 3) + j, 21, n,
			i + (j * 2) + 4, i + (j * 2) + 3, change_eq, 0, 127);
		n = i + (j * 2) + 5;
		if (j == 3)
			n--;
		widget_create_thumbbar(widgets_preferences + i + 3 + (j * 2), 53, 23 + (i * 3) + j, 21, i + (j * 2) + 1,
			n, i + (j * 2) + 4, change_eq, 0, 127);
	}
	/* default EQ setting */
	...
	ramp_group[0] = i + 10;
	ramp_group[1] = i + 11;
	widget_create_togglebutton(widgets_preferences + i + 10, 33, 29 + i * 3, 9, i + 9, i + 12, i + 10, i + 11,
		i + 11, change_mixer, "Enabled", 2, ramp_group);

	widget_create_togglebutton(widgets_preferences + i + 11, 46, 29 + i * 3, 9, i + 9, i + 12, i + 10, i + 13,
		i + 13, change_mixer, "Disabled", 1, ramp_group);

	widget_create_button(widgets_preferences + i + 12, 2, 44, 27, i + 10, i + 12, i + 12, i + 13, i + 13,
		save_config_now, "Save Output Configuration", 2);

	widget_create_listbox(widgets_preferences + i + 13, audio_device_list_size_, audio_device_list_toggled_,
		audio_device_list_name_, NULL, audio_device_list_activate_, NULL, audio_device_focus_offsets_,
	...
	widget_create_listbox(widgets_preferences + i + 14, audio_driver_list_size_, audio_driver_list_toggled_,
		audio_driver_list_name_, NULL, audio_driver_list_activate_, NULL, audio_driver_focus_offsets_,
	...
	widget_create_button(widgets_preferences + i + 15, 56, 44, 20, i + 10, i + 12, i + 12, i + 13, i + 13,
		open_control_panel, "Open Control Panel", 2);
}
