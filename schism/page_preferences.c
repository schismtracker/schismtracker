...
static const char *interpolation_modes[] = {"Non-Interpolated", "Linear", "Cubic Spline", "8-Tap FIR Filter", NULL};
...
static int ramp_group[] = {
	/* not const because it is modified */
	-1,
	-1,
	-1,
};
...
static void preferences_draw_const(void)
{
	...
	draw_box(
		AUDIO_DEVICE_BOX_X - 1, AUDIO_DEVICE_BOX_Y - 1, AUDIO_DEVICE_BOX_END_X + 1, AUDIO_DEVICE_BOX_END_Y + 1,
		BOX_THICK | BOX_INNER | BOX_INSET);

	draw_text("Available Audio Drivers", AUDIO_DRIVER_BOX_X, AUDIO_DRIVER_BOX_Y - 2, 0, 2);
	draw_box(
		AUDIO_DRIVER_BOX_X - 1, AUDIO_DRIVER_BOX_Y - 1, AUDIO_DRIVER_BOX_END_X + 1, AUDIO_DRIVER_BOX_END_Y + 1,
		BOX_THICK | BOX_INNER | BOX_INSET);
...
static void preferences_set_page(void)
{
	...
	for (j = 0; j < 4; j++) {
		widgets_preferences[i + 2 + (j * 2)].d.thumbbar.value = audio_settings.eq_freq[j];
		widgets_preferences[i + 3 + (j * 2)].d.thumbbar.value = audio_settings.eq_gain[j];
	}

	widgets_preferences[i + 10].d.togglebutton.state = audio_settings.no_ramping ? 0 : 1;
	widgets_preferences[i + 11].d.togglebutton.state = audio_settings.no_ramping ? 1 : 0;
}
...
static void audio_device_list_draw()
{
	...
	draw_fill_chars(
		AUDIO_DEVICE_BOX_X, AUDIO_DEVICE_BOX_Y, AUDIO_DEVICE_BOX_END_X, AUDIO_DEVICE_BOX_END_Y, DEFAULT_FG, 0);
...
\
  draw_text_utf8_len( \
	  !strcmp(current_audio_device, name) ? "*" : " ", 1, AUDIO_DEVICE_BOX_X, AUDIO_DEVICE_BOX_Y + o, fg, bg); \
  draw_text_utf8_len(name, AUDIO_DEVICE_BOX_WIDTH - 1, AUDIO_DEVICE_BOX_X + 1, AUDIO_DEVICE_BOX_Y + o, fg, bg); \
  o++; \
 } while (0)

	if (top_audio_device < 1) DRAW_DEVICE("default");
...
static int audio_device_list_handle_key_on_list(struct key_event *k)
{
	...
	case SDLK_RETURN:
		...
		break;
	case SDLK_TAB:
		if (!(k->mod & KMOD_SHIFT || NO_MODIFIER(k->mod))) return 0;
...
static void audio_driver_list_draw()
{
	...
	draw_fill_chars(
		AUDIO_DRIVER_BOX_X, AUDIO_DRIVER_BOX_Y, AUDIO_DRIVER_BOX_END_X, AUDIO_DRIVER_BOX_END_Y, DEFAULT_FG, 0);
	...
	for (n = top_audio_driver; n < num_drivers && o < AUDIO_DRIVER_BOX_HEIGHT; n++) {
		...
		draw_text_utf8_len(
			!strcmp(current_audio_driver, name) ? "*" : " ", 1, AUDIO_DRIVER_BOX_X, AUDIO_DRIVER_BOX_Y + o, fg, bg);
...
static int audio_driver_list_handle_key_on_list(struct key_event *k)
{
	...
	case MOUSE_CLICK:
		...
		if (k->x < AUDIO_DRIVER_BOX_X || k->y < AUDIO_DRIVER_BOX_Y || k->y > AUDIO_DRIVER_BOX_END_Y
		    || k->x > AUDIO_DRIVER_BOX_END_X)
			return 0;
		new_driver = top_audio_driver + (k->y - AUDIO_DRIVER_BOX_Y);
		if (k->mouse == MOUSE_DBLCLICK || new_driver == selected_audio_driver) load_selected_driver = 1;
		break;
	case MOUSE_SCROLL_UP: new_driver -= MOUSE_SCROLL_LINES; break;
	case MOUSE_SCROLL_DOWN: new_driver += MOUSE_SCROLL_LINES; break;
	default:
		if (k->state == KEY_RELEASE) return 0;
	}
...
void preferences_load_page(struct page *page)
{
	...
	widgets_preferences[0].next.left = widgets_preferences[0].next.right = widgets_preferences[0].next.tab =
		widgets_preferences[0].next.backtab = widgets_preferences[1].next.left = widgets_preferences[1].next.right =
			widgets_preferences[1].next.tab = widgets_preferences[1].next.backtab = interp_modes + 13;
	...
	for (i = 0; interpolation_modes[i]; i++) {
		...
		widget_create_togglebutton(
			widgets_preferences + i + 2, 6, 20 + (i * 3), 26, i + 1, i + 3, i + 2, interp_modes + 11, i + 3,
			change_mixer, ptr, 2, interp_group);
		if (i < 1) widgets_preferences[i + 2].next.left = widgets_preferences[i + 2].next.right = interp_modes + 13;
		else if (i < 4)
			widgets_preferences[i + 2].next.left = widgets_preferences[i + 2].next.right = interp_modes + 14;
	}
	...
	for (j = 0; j < 4; j++) {
		...
		widget_create_thumbbar(
			widgets_preferences + i + 2 + (j * 2), 26, 23 + (i * 3) + j, 21, n, i + (j * 2) + 4, i + (j * 2) + 3,
			change_eq, 0, 127);
		n = i + (j * 2) + 5;
		if (j == 3) n--;
		widget_create_thumbbar(
			widgets_preferences + i + 3 + (j * 2), 53, 23 + (i * 3) + j, 21, i + (j * 2) + 1, n, i + (j * 2) + 4,
			change_eq, 0, 127);
	}
	/* default EQ setting */
	...
	ramp_group[1] = i + 11;
	widget_create_togglebutton(
		widgets_preferences + i + 10, 33, 29 + i * 3, 9, i + 9, i + 12, i + 10, i + 11, i + 11, change_mixer, "Enabled",
		2, ramp_group);

	widget_create_togglebutton(
		widgets_preferences + i + 11, 46, 29 + i * 3, 9, i + 9, i + 12, i + 10, i + 13, i + 13, change_mixer,
		"Disabled", 1, ramp_group);

	widget_create_button(
		widgets_preferences + i + 12, 2, 44, 27, i + 10, i + 12, i + 12, i + 13, i + 13,
		(void (*)(void))save_config_now, "Save Output Configuration", 2);

	widget_create_other(
		widgets_preferences + i + 13, 0, audio_device_list_handle_key_on_list, NULL, audio_device_list_draw);
	...
	widget_create_other(
		widgets_preferences + i + 14, 0, audio_driver_list_handle_key_on_list, NULL, audio_driver_list_draw);
