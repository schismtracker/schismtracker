...
static const char *const time_displays[] = {"Off",     "Play / Elapsed", "Play / Clock", "Play / Off",
											"Elapsed", "Clock",          "Absolute",     NULL};
static const char *const vis_styles[] = {"Off", "Memory Stats", "Oscilloscope", "VU Meter", "Monoscope", "Spectrum",
										 NULL};
...
static const char *const bit_rates[] = {
...
static void video_mode_cancel(UNUSED void *ign)
{
	...
	if (video_is_fullscreen() != video_revert_fs) video_fullscreen(-1);
	palette_apply();
	font_init();
	config_set_page();
	status.flags |= NEED_UPDATE;
...
static void video_change_dialog(void)
{
	...
	create_button(video_dialog_widgets + 0, 28, 28, 8, 0, 0, 0, 1, 1, dialog_yes_NULL, "OK", 4);
	create_button(video_dialog_widgets + 1, 42, 28, 8, 1, 1, 0, 1, 0, dialog_cancel_NULL, "Cancel", 2);
...
static void change_video_settings(void)
{
	...
	if (new_fs_flag != video_is_fullscreen()) toggle_display_fullscreen();
	palette_apply();
	font_init();
}
...
void config_load_page(struct page *page)
{
	...
	create_thumbbar(widgets_config + 0, 18, 15, 17, 0, 1, 1, change_mixer_limits, 4, 256);
	create_numentry(widgets_config + 1, 18, 16, 7, 0, 2, 2, change_mixer_limits, 4000, 192000, &sample_rate_cursor);
	create_menutoggle(widgets_config + 2, 18, 17, 1, 3, 2, 2, 3, change_mixer_limits, bit_rates);
	create_menutoggle(widgets_config + 3, 18, 18, 2, 4, 3, 3, 4, change_mixer_limits, output_channels);
	////
	create_menutoggle(widgets_config + 4, 18, 20, 3, 5, 4, 4, 5, change_ui_settings, vis_styles);
	create_toggle(widgets_config + 5, 18, 21, 4, 6, 5, 5, 6, change_ui_settings);
	create_menutoggle(widgets_config + 6, 18, 22, 5, 7, 6, 6, 7, change_ui_settings, sharp_flat);
	create_menutoggle(widgets_config + 7, 18, 23, 6, 8, 7, 7, 8, change_ui_settings, time_displays);
	////
	create_menutoggle(widgets_config + 8, 18, 25, 7, 11, 8, 8, 11, change_ui_settings, midi_modes);
	////
	create_togglebutton(
...
#ifdef SCHISM_WIN32
	create_togglebutton(
		widgets_config + 14, 44, 34, 5, 8, 9, 11, 10, 10, change_menu_bar_settings, "Yes", 2, video_menu_bar_group);
	create_togglebutton(
		widgets_config + 15, 54, 34, 5, 10, 10, 9, 10, 0, change_menu_bar_settings, "No", 2, video_menu_bar_group);
#endif
}
