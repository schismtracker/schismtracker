...
static struct menu main_menu = {
	...
	.items =
		{
				"File Menu...", "Playback Menu...",
				"View Patterns        (F2)", "Sample Menu...",
				"Instrument Menu...", "View Orders/Panning (F11)",
				"View Variables      (F12)", "Message Editor (Shift-F9)",
				"Settings Menu...", "Help!                (F1)",
				},
	.selected_item = 0,
	.active_item = -1,
...
static struct menu file_menu = {
	...
	.items =
		{
				"Load...           (F9)", "New...        (Ctrl-N)",
				"Save Current  (Ctrl-S)", "Save As...       (F10)",
				"Export...  (Shift-F10)", "Message Log (Ctrl-F11)",
				"Quit          (Ctrl-Q)", },
	.selected_item = 0,
	.active_item = -1,
...
static struct menu playback_menu = {
	...
	.items =
		{
				"Show Infopage          (F5)", "Play Song         (Ctrl-F5)",
				"Play Pattern           (F6)", "Play from Order  (Shift-F6)",
				"Play from Mark/Cursor  (F7)", "Stop                   (F8)",
				"Reinit Soundcard   (Ctrl-I)", "Driver Screen    (Shift-F5)",
...
static struct menu instrument_menu = {
	...
	.items =
		{
				"Instrument List          (F4)", "Instrument Library  (Ctrl-F4)",
				},
	.selected_item = 0,
	.active_item = -1,
...
static struct menu settings_menu = {
 ...
 /* num_items is fiddled with when the menu is loaded (if there's no window manager,
	...
	.items =
		{
				"Preferences             (Shift-F5)", "MIDI Configuration      (Shift-F1)",
				"System Configuration     (Ctrl-F1)", "Palette Editor          (Ctrl-F12)",
				"Font Editor            (Shift-F12)", "Toggle Fullscreen (Ctrl-Alt-Enter)",
				},
	.selected_item = 0,
	.active_item = -1,
...
static void _draw_menu(struct menu *menu)
{
	...
	while (n--) {
		draw_box(
			2 + menu->x, 4 + menu->y + 3 * n, 5 + menu->x + menu->w, 6 + menu->y + 3 * n,
			BOX_THIN | BOX_CORNER | (n == menu->active_item ? BOX_INSET : BOX_OUTSET));

		draw_text_len(menu->items[n], menu->w, 4 + menu->x, 5 + menu->y + 3 * n, (n == menu->selected_item ? 3 : 0), 2);
	...
	draw_box(menu->x + 1, menu->y + 1, menu->x + menu->w + 6, menu->y + h - 2, BOX_THIN | BOX_OUTER | BOX_FLAT_DARK);
...
void menu_draw(void)
{
	...
	if (current_menu[1]) _draw_menu(current_menu[1]);
}
...
void menu_hide(void)
{
	...
	if (current_menu[1]) current_menu[1]->active_item = -1;
...
static void main_menu_selected_cb(void)
{
	...
	case 8: /* settings menu */
		...
		if (video_is_wm_available()) settings_menu.num_items = 6;
	...
	if (k->mouse) {
		if (k->mouse == MOUSE_CLICK || k->mouse == MOUSE_DBLCLICK) {
			...
			if (k->x >= menu->x + 2 && k->x <= menu->x + menu->w + 5 && k->y >= menu->y + 4
				...
				k->state == KEY_RELEASE
				&& (k->x < menu->x || k->x > 7 + menu->x + menu->w || k->y < menu->y || k->y >= 5 + menu->y + h)) {
				/* get rid of the menu */
				current_menu[1] = NULL;
