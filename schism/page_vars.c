...
{
	char* shortcut_text = (char*)global_keybinds_list.global.song_variables.shortcut_text;

	if(shortcut_text[0]) {
		page->title = str_concat_three("Song Variables & Directory Configuration (", shortcut_text, ")", 0);
	} else {
		page->title = "Song Variables & Directory Configuration";
	}
