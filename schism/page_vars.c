...
void song_vars_sync_stereo(void)
{
	...
	if (song_is_stereo()) togglebutton_set(widgets_vars, 10, 0);
	else togglebutton_set(widgets_vars, 11, 0);
}
...
static void song_changed_cb(void)
{
	...
	if (song_is_instrument_mode()) togglebutton_set(widgets_vars, 8, 0);
	else togglebutton_set(widgets_vars, 9, 0);

	if (song_is_stereo()) togglebutton_set(widgets_vars, 10, 0);
	else togglebutton_set(widgets_vars, 11, 0);

	if (song_has_linear_pitch_slides()) togglebutton_set(widgets_vars, 12, 0);
	else togglebutton_set(widgets_vars, 13, 0);
...
void song_vars_load_page(struct page *page)
{
	...
	/* 14-16 = directories */
	create_textentry(widgets_vars + 14, 13, 42, 65, 12, 15, 15, dir_modules_changed, cfg_dir_modules, PATH_MAX);
	create_textentry(widgets_vars + 15, 13, 43, 65, 14, 16, 16, dir_samples_changed, cfg_dir_samples, PATH_MAX);
	create_textentry(widgets_vars + 16, 13, 44, 65, 15, 17, 17, dir_instruments_changed, cfg_dir_instruments, PATH_MAX);
