...
static void zxx_setpos(int pos)
{
	...
	if (zxx_top == pos) return;
	zxx_top = pos;
	for (i = 0; i < 7; i++) widgets_midiout[25 + i].d.textentry.text = editcfg.zxx[zxx_top + i];
...
static int pre_handle_key(struct key_event *k)
{
	if (*selected_widget == 25 && k->sym == SDLK_UP) {
		...
		if (zxx_top == 0) return 0; /* let the normal key handler catch it and change focus */
...
void midiout_load_page(struct page *page)
{
	...
	for (i = 0; i < 9; i++) {
		widget_create_textentry(
			widgets_midiout + i, 17, 13 + i, 43, (i == 0 ? 0 : (i - 1)), i + 1, 9, copy_out, editcfg_top[i], 31);
	}
	for (i = 0; i < 16; i++) {
		widget_create_textentry(
			widgets_midiout + 9 + i, 17, 24 + i, 43, 9 + i - 1, 9 + i + 1, 25, copy_out, editcfg.sfx[i], 31);
	}
	for (i = 0; i < 7; i++) {
		widget_create_textentry(
			widgets_midiout + 25 + i, 17, 42 + i, 43, 25 + i - 1, 25 + ((i == 6) ? 6 : (i + 1)), 0, copy_out,
			editcfg.zxx[i], 31);
	}
}
