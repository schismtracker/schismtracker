...
void memused_get_pattern_saved(unsigned int *a, unsigned int *b)
{
	...
	if (b) {
		for (i = 0; i < 10; i++) {
			if (undo_history[i].data) *b = (*b) + undo_history[i].rows;
		}
	}
	if (a) {
...
#define TRACK_VIEW(n) \
 { \
  n, draw_channel_header_##n, draw_note_##n, draw_mask_##n \
 }
...
void pattern_editor_display_options(void)
{
	...
	if (options_widgets[0].width == 0) {
		...
		widget_create_togglebutton(options_widgets + 5, 40, 38, 8, 4, 7, 6, 6, 6, NULL, "Link", 3, options_link_split);
		widget_create_togglebutton(options_widgets + 6, 52, 38, 9, 4, 7, 5, 5, 5, NULL, "Split", 3, options_link_split);
	...
	dialog =
		dialog_create_custom(10, 18, 60, 26, options_widgets, 8, options_selected_widget, options_draw_const, NULL);
...
static void multichannel_close(UNUSED void *data)
{
	...
	for (i = 0; i < 64; i++) {
		...
		if (channel_multi[i]) m = 1;
	}
	if (m) channel_multi_enabled = 1;
}
static int multichannel_handle_key(struct key_event *k)
{
	if (k->sym == SDLK_n) {
		if ((k->mod & KMOD_ALT) && k->state == KEY_PRESS) dialog_yes(NULL);
		else if (NO_MODIFIER(k->mod) && k->state == KEY_RELEASE) dialog_cancel(NULL);
		return 1;
	}
	return 0;
}
...
static void pattern_editor_display_multichannel(void)
{
	...
	for (i = 0; i < 64; i++) {
		widget_create_toggle(
			multichannel_widgets + i, 20 + ((i / 16) * 16), /* X */
			22 + (i % 16),                                  /* Y */
	...
	dialog = dialog_create_custom(7, 18, 66, 25, multichannel_widgets, 65, 0, multichannel_draw_const, NULL);
...
static int pattern_selection_system_paste(UNUSED int cb, const void *data)
{
	...
	while (*str) {
		...
		str += 3;
		/* instrument number */
		if (sscanf(str, "%02u", &scantmp) == 1) n.instrument = scantmp;
		else n.instrument = 0;
		str += 2;
		while (*str) {
			if (*str == '|' || *str == '\r' || *str == '\n') break;
			if (!str[0] || !str[1] || !str[2]) break;
			if (*str >= 'a' && *str <= 'z') {
				if (sscanf(str + 1, "%02u", &scantmp) == 1) n.volparam = scantmp;
				else n.volparam = 0;
				switch (*str) {
				...
				n.effect = fx_map(*str);
				if (sscanf(str + 1, "%02X", &scantmp) == 1) n.param = scantmp;
				else n.param = 0;
			}
			str += 3;
		}
...
static void pattern_selection_system_copyout(void)
{
	...
	*/
	...
	len = 21;
	for (y = selection.first_row; y <= selection.last_row && y < total_rows; y++) {
		cur_note = pattern + 64 * y + selection.first_channel - 1;
		for (x = selection.first_channel; x <= selection.last_channel; x++) {
			str[len] = '|';
			len++;
			if (cur_note->note == 0) {
			...
			len += 3;
			if (cur_note->instrument) sprintf(str + len, "%02d", cur_note->instrument);
			...
			if (str[len] == '.' || str[len] == '?') {
				str[len] = '.';
				if (!cur_note->param) str[len + 1] = str[len + 2] = '.';
			}
			len += 3;
...
static void fast_volume_toggle(void)
{
	...
	if (fast_volume_mode) {
		...
		dialog = dialog_create_custom(22, 25, 36, 11, volume_setup_widgets, 3, 0, fast_volume_setup_draw_const, NULL);
...
void cfg_save_patedit(cfg_file_t *cfg)
{
	...
	for (n = 0; n < 64; n++) s[n] = track_view_scheme[n] + 'a';
	s[64] = 0;

	cfg_set_string(cfg, "Pattern Editor", "track_view_scheme", s);
	for (n = 0; n < 64; n++) s[n] = (channel_multi[n]) ? 'M' : '-';
	s[64] = 0;
...
void cfg_load_patedit(cfg_file_t *cfg)
{
	...
	if (cfg_get_number(cfg, "Pattern Editor", "crayola_mode", 0)) status.flags |= CRAYOLA_MODE;
	else status.flags &= ~CRAYOLA_MODE;
	...
	if (n < 64) memset(track_view_scheme + n, r, 64 - n);
	...
	for (n = 0; n < 64; n++) {
		...
		if (channel_multi[n]) channel_multi_enabled = 1;
	}

	recalculate_visible_area();
	pattern_editor_reposition();
	if (status.current_page == PAGE_PATTERN_EDITOR) status.flags |= NEED_UPDATE;
}
...
{
	return (
		SELECTION_EXISTS && chan >= selection.first_channel && chan <= selection.last_channel
...
static void selection_erase(void)
{
	...
	pated_history_add(
		"Undo block cut                 (Alt-Z)", selection.first_channel - 1, selection.first_row,
	...
	if (selection.first_channel == 1 && selection.last_channel == 64) {
		memset(
			pattern + 64 * selection.first_row, 0,
			(selection.last_row - selection.first_row + 1) * 64 * sizeof(song_note_t));
	} else {
...
static void selection_set_sample(void)
{
	...
	status.flags |= SONG_NEEDS_SAVE;
	pated_history_add(
		"Undo set sample/instrument     (Alt-S)", selection.first_channel - 1, selection.first_row,
		(selection.last_channel - selection.first_channel) + 1, (selection.last_row - selection.first_row) + 1);
	if (SELECTION_EXISTS) {
		for (row = selection.first_row; row <= selection.last_row; row++) {
			note = pattern + 64 * row + selection.first_channel - 1;
...
static void selection_swap(void)
{
	...
	affected_height = MAX(selection.last_row, current_row + sel_rows - 1) - MIN(selection.first_row, current_row) + 1;
	...
	pated_history_add(
		"Undo swap block                (Alt-Y)", MIN(selection.first_channel, current_channel) - 1,
...
static void selection_set_volume(void)
{
	...
	pated_history_add(
		"Undo set volume/panning        (Alt-V)", selection.first_channel - 1, selection.first_row,
		(selection.last_channel - selection.first_channel) + 1, (selection.last_row - selection.first_row) + 1);
...
static void selection_slide_volume(void)
{
	...
	/* can't slide one row */
	if (selection.first_row == selection.last_row) return;

	status.flags |= SONG_NEEDS_SAVE;

	pated_history_add(
		"Undo volume or panning slide   (Alt-K)", selection.first_channel - 1, selection.first_row,
		(selection.last_channel - selection.first_channel) + 1, (selection.last_row - selection.first_row) + 1);
	...
	for (chan = selection.first_channel; chan <= selection.last_channel; chan++) {
		...
		if (lve == VOLFX_NONE) {
			if (last_note->instrument == 0) continue;
			lve = VOLFX_VOLUME;
		...
		for (row = selection.first_row; row <= selection.last_row; row++, note += 64) {
			note->voleffect = ve;
			note->volparam =
				(((last - first) * (row - selection.first_row) / (selection.last_row - selection.first_row)) + first);
		}
	}
	pattern_selection_system_copyout();
}
...
static void selection_wipe_volume(int reckless)
{
	...
	pated_history_add(
		...
		selection.first_channel - 1, selection.first_row, (selection.last_channel - selection.first_channel) + 1,
...
static void selection_vary(int fast, int depth, int how)
{
	...
	pated_history_add(
		vary_how, selection.first_channel - 1, selection.first_row,
		(selection.last_channel - selection.first_channel) + 1, (selection.last_row - selection.first_row) + 1);
...
static void selection_amplify(int percentage)
{
	...
	pated_history_add(
		"Undo volume amplification      (Alt-J)", selection.first_channel - 1, selection.first_row,
		(selection.last_channel - selection.first_channel) + 1, (selection.last_row - selection.first_row) + 1);
	...
	for (row = selection.first_row; row <= selection.last_row; row++) {
		...
		for (chan = selection.first_channel; chan <= selection.last_channel; chan++, note++) {
			if (note->voleffect == VOLFX_NONE && note->instrument != 0) {
				/* Modplug hack: volume bit shift */
				if (song_is_instrument_mode()) volume = 64; /* XXX */
				else volume = song_get_sample(note->instrument)->volume >> 2;
...
static void selection_slide_effect(void)
{
	...
	pated_history_add(
		"Undo effect data slide         (Alt-X)", selection.first_channel - 1, selection.first_row,
		(selection.last_channel - selection.first_channel) + 1, (selection.last_row - selection.first_row) + 1);
...
static void selection_wipe_effect(void)
{
	...
	pated_history_add(
		"Recover effects/effect data  (2*Alt-X)", selection.first_channel - 1, selection.first_row,
		(selection.last_channel - selection.first_channel) + 1, (selection.last_row - selection.first_row) + 1);
...
static void pattern_insert_rows(int what_row, int num_rows, int first_channel, int chan_width)
{
	...
	status.flags |= SONG_NEEDS_SAVE;
	if (first_channel < 1) first_channel = 1;
	if (chan_width + first_channel - 1 > 64) chan_width = 64 - first_channel + 1;

	if (num_rows + what_row > total_rows) num_rows = total_rows - what_row;

	if (first_channel == 1 && chan_width == 64) {
		memmove(
			pattern + 64 * (what_row + num_rows), pattern + 64 * what_row,
		...
		for (row = total_rows - num_rows - 1; row >= what_row; row--) {
			memmove(
				pattern + 64 * (row + num_rows) + first_channel - 1, pattern + 64 * row + first_channel - 1,
...
static void pattern_delete_rows(int what_row, int num_rows, int first_channel, int chan_width)
{
	...
	if (first_channel == 1 && chan_width == 64) {
		memmove(
			pattern + 64 * what_row, pattern + 64 * (what_row + num_rows),
			64 * sizeof(song_note_t) * (total_rows - what_row - num_rows));
		...
		for (row = what_row; row <= total_rows - num_rows - 1; row++) {
			memmove(
				pattern + 64 * row + first_channel - 1, pattern + 64 * (row + num_rows) + first_channel - 1,
				chan_width * sizeof(song_note_t));
		}
		/* clear the last rows */
...
static void pated_history_clear(void)
{
	...
	int i;
	for (i = 0; i < 10; i++) {
		if (undo_history[i].snap_op_allocated) free((void *)undo_history[i].snap_op);
...
static void snap_paste(struct pattern_snap *s, int x, int y, int xlate)
{
	...
	num_rows = song_get_pattern(current_pattern, &pattern);
	num_rows -= y;
	if (s->rows < num_rows) num_rows = s->rows;
	if (num_rows <= 0) return;

	chan_width = s->channels;
	if (chan_width + x >= 64) chan_width = 64 - x;

	for (row = 0; row < num_rows; row++) {
		p_note = pattern + 64 * (y + row) + x;
		memcpy(pattern + 64 * (y + row) + x, s->data + s->channels * row, chan_width * sizeof(song_note_t));
		if (!xlate) continue;
...
static void snap_copy(struct pattern_snap *s, int x, int y, int width, int height)
{
	...
	if (x == 0 && width == 64) {
		...
		for (row = 0; row < s->rows && row < total_rows; row++) {
			memcpy(s->data + s->channels * row, pattern + 64 * (row + s->y) + s->x, s->channels * sizeof(song_note_t));
		}
	}
}
...
static void clipboard_copy(int honor_mute)
{
	...
	snap_copy(
		&clipboard, selection.first_channel - 1, selection.first_row,
		(selection.last_channel - selection.first_channel) + 1, (selection.last_row - selection.first_row) + 1);
...
static void clipboard_paste_overwrite(int suppress, int grow)
{
	...
	if (clipboard.rows < num_rows) num_rows = clipboard.rows;
	...
	if (chan_width + current_channel > 64) chan_width = 64 - current_channel + 1;

	if (!suppress) {
		pated_history_add_grouped(
			"Replace overwritten data       (Alt-O)", current_channel - 1, current_row, chan_width, num_rows);
	}
	...
	for (row = 0; row < num_rows; row++) {
		for (chan = 0; chan < chan_width; chan++) {
			...
			if (prec) {
				...
				if (c_note[chan].instrument != 0) p_note[chan].instrument = c_note[chan].instrument;
				...
				if (c_note[chan].param != 0) p_note[chan].param = c_note[chan].param;
			} else {
				if (p_note[chan].note == 0) {
					set_note_note(p_note + chan, c_note[chan].note, xlate);
				}
				if (p_note[chan].instrument == 0) p_note[chan].instrument = c_note[chan].instrument;
				...
				if (p_note[chan].param == 0) p_note[chan].param = c_note[chan].param;
			}
		}
		p_note += 64;
		c_note += clipboard.channels;
...
static void pattern_editor_reposition(void)
{
	...
	if (current_channel < top_display_channel) top_display_channel = current_channel;
	...
	if (centralise_cursor) {
		if (current_row <= 16) top_display_row = 0;
		else if (current_row + 15 > total_rows) top_display_row = total_rows - 31;
		else top_display_row = current_row - 16;
	} else {
		/* This could be written better. */
		if (current_row < top_display_row) top_display_row = current_row;
		else if (current_row > top_display_row + 31) top_display_row = current_row - 31;
		if (top_display_row + 31 > total_rows) top_display_row = total_rows - 31;
	}
...
static void recalculate_visible_area(void)
{
	...
	for (n = 0; n < 64; n++) {
		...
		if (new_width > 72) break;
		visible_width = new_width;
		if (draw_divisions) visible_width++;
	}
	...
	if (top_display_channel > 64 - visible_channels + 1) top_display_channel = 64 - visible_channels + 1;
}
	...
	for (chan = top_display_channel, chan_pos = 0; chan_pos < visible_channels; chan++, chan_pos++) {
		...
		for (row = top_display_row, row_pos = 0; row_pos < 32 && row < total_rows; row++, row_pos++) {
			...
			if (is_in_selection(chan, row)) {
				...
				if (highlight_current_row && row == current_row) bg = 1;
				else if (ROW_IS_MAJOR(row)) bg = 14;
				else if (ROW_IS_MINOR(row)) bg = 15;
				else bg = 0;
			}
			...
			int cpos;
			if ((row == current_row)
			    && ((current_position > 0 || template_mode == TEMPLATE_OFF || (status.flags & SHIFT_KEY_DOWN)) ?
			            (chan == current_channel) :
			            (chan >= current_channel
			             && chan < (current_channel + (clipboard.data ? clipboard.channels : 1))))) {
			...
			if (draw_divisions && chan_pos < visible_channels - 1) {
				if (is_in_selection(chan, row)) bg = 0;
...
static void transpose_notes(int amount)
{
	...
	if (SELECTION_EXISTS) {
		for (row = selection.first_row; row <= selection.last_row; row++) {
			note = pattern + 64 * row + selection.first_channel - 1;
			for (chan = selection.first_channel; chan <= selection.last_channel; chan++) {
				if (note->note > 0 && note->note < 121) note->note = CLAMP(note->note + amount, 1, 120);
				note++;
			}
		}
	} else {
...
static int handle_volume(song_note_t *note, struct key_event *k, int pos)
{
	...
	note->voleffect = fx;
	if (fx == VOLFX_VOLUME || fx == VOLFX_PANNING) note->volparam = CLAMP(vol, 0, 64);
	else note->volparam = CLAMP(vol, 0, 9);
	return 1;
}
	...
	if (NOTE_IS_NOTE(note)) {
		if (template_mode) {
			...
			if (clipboard.channels < 1 || clipboard.rows < 1 || !clipboard.data) {
				...
				widget_create_button(template_error_widgets + 0, 36, 32, 6, 0, 0, 0, 0, 0, dialog_yes_NULL, "OK", 3);
				dialog_create_custom(20, 23, 40, 12, template_error_widgets, 1, 0, template_error_draw, NULL);
				r = 0;
			} else {
				i = note - q->note;
		...
		for (i = 0; i < channels && i + channel <= 64; i++) {
			...
			if (!force && cur_note->note) continue;
...
static int pattern_editor_insert_midi(struct key_event *k)
{
	...
	if (!(midi_flags & MIDI_PITCHBEND) || midi_pitch_depth == 0 || k->midi_bend == 0) {
		if (k->state == KEY_RELEASE && k->midi_note > -1 && cur_note->instrument > 0) {
			song_keyrecord(
				cur_note->instrument, cur_note->instrument, cur_note->note, v, c + 1, cur_note->effect,
				cur_note->param);
			pattern_selection_system_copyout();
		}
		return -1;
	...
	for (c = 0; c < 64; c++) {
		if ((channel_multi[c] & 1) && (channel_multi[c] & (~1))) {
			...
			if (k->midi_note == -1 || k->state == KEY_RELEASE) continue;
			if (cur_note->instrument < 1) continue;
			if (cur_note->voleffect == VOLFX_VOLUME) v = cur_note->volparam;
			else v = -1;
			song_keyrecord(
				cur_note->instrument, cur_note->instrument, cur_note->note, v, c + 1, cur_note->effect,
				cur_note->param);
...
static int pattern_editor_insert(struct key_event *k)
{
	...
	if (k->state == KEY_RELEASE && current_position) return 0;
...
static int pattern_editor_handle_ctrl_key(struct key_event *k)
{
	...
	n = numeric_key_event(k, 0);
	if (n > -1) {
		if (n < 0 || n >= NUM_TRACK_VIEWS) return 0;
		if (k->state == KEY_RELEASE) return 1;
		if (k->mod & KMOD_SHIFT) {
			set_view_scheme(n);
		} else {
...
static int pattern_editor_handle_key_default(struct key_event *k)
{
	...
	if (song_get_mode() & (MODE_PLAYING | MODE_PATTERN_LOOP) && playback_tracing && k->is_repeat) return 0;
...
static int pattern_editor_handle_key(struct key_event *k)
{
	...
	if (k->mouse != MOUSE_NONE) {
		...
		if (k->mouse != MOUSE_CLICK && k->mouse != MOUSE_DBLCLICK) return 1;
		...
		for (n = top_display_channel, nx = 0; nx <= visible_channels; n++, nx++) {
			...
			if (((n == top_display_channel && shift_selection.in_progress) || k->x >= basex)
			    && ((n == visible_channels && shift_selection.in_progress) || k->x < basex + track_view->width)) {
...
static int pattern_editor_handle_key_cb(struct key_event *k)
{
	...
	if (k->mod & KMOD_ALT) ret = pattern_editor_handle_alt_key(k);
	else if (k->mod & KMOD_CTRL) ret = pattern_editor_handle_ctrl_key(k);
	else ret = pattern_editor_handle_key(k);
	...
	pattern_editor_reposition();
	if (k->mod & KMOD_SHIFT) shift_selection_update();
