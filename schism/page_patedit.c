...
void pattern_editor_display_options(void)
{
	...
	if (options_widgets[0].width == 0) {
		...
		create_togglebutton(options_widgets + 5, 40, 38, 8, 4, 7, 6, 6, 6, NULL, "Link", 3, options_link_split);
...
static int pattern_selection_system_paste(UNUSED int cb, const void *data)
{
	...
	while (*str) {
		...
		str += 3;
		/* instrument number */
		if (sscanf(str, "%02d", &scantmp) == 1) n.instrument = scantmp;
		else n.instrument = 0;
		str += 2;
		while (*str) {
			if (*str == '|' || *str == '\r' || *str == '\n') break;
	...
	if (NOTE_IS_NOTE(note)) {
		if (template_mode) {
			...
			if (clipboard.channels < 1 || clipboard.rows < 1 || !clipboard.data) {
				...
				create_button(template_error_widgets + 0, 36, 32, 6, 0, 0, 0, 0, 0, dialog_yes_NULL, "OK", 3);
