...
static void load_sample_draw_const(void)
{
	...
	if (need_trigger > -1) {
		if (fake_slot > -1) {
			if (need_keyoff > -1)
				song_keyup(fake_slot, 0, need_keyoff);
			need_keyoff = need_trigger;
			song_keydown(fake_slot, 0, need_trigger, 64, KEYDOWN_CHAN_CURRENT);
		}
		need_trigger = -1;
	}
}
...
static void load_sample_handle_key(struct key_event * k)
{
	...
	if (fake_slot > -1) {
		if ((status.flags & CLASSIC_MODE) || !song_is_multichannel_mode()) {
			...
			if (k->state) {
				song_keyup(fake_slot, 0, n);
				status.last_keysym = 0;
			} else if (!k->is_repeat) {
				song_keydown(fake_slot, 0, n, v, KEYDOWN_CHAN_CURRENT);
			}
			need_trigger = -1;
		}
	}
