...
static void read_directory(void)
{
	...
	clear_directory();
	if (os_stat(cfg_dir_samples, &st) < 0) directory_mtime = 0;
	...
	if (dmoz_read(cfg_dir_samples, &flist, NULL, dmoz_read_sample_library) < 0) log_perror(cfg_dir_samples);
...
static void _common_set_page(void)
{
	...
	if (flist.num_files > 0 && (status.flags & DIR_SAMPLES_CHANGED) == 0 && os_stat(cfg_dir_samples, &st) == 0
			...
			if (file->base && search_pos > -1) {
				if (strncasecmp(out, search_str, search_pos) == 0) {
					for (i = 0; i < search_pos; i++) {
						if (tolower(out[i]) != tolower(search_str[i])) break;
...
static void finish_load(int cur)
{
	...
	if (smp->flags & CHN_STEREO) {
		struct dialog *dd;
		create_button(stereo_cvt_widgets + 0, 27, 30, 6, 0, 0, 2, 1, 1, stereo_cvt_complete_left, "Left", 2);

		create_button(stereo_cvt_widgets + 1, 37, 30, 6, 1, 1, 0, 2, 2, stereo_cvt_complete_both, "Both", 2);

		create_button(stereo_cvt_widgets + 2, 47, 30, 6, 2, 2, 1, 0, 0, stereo_cvt_complete_right, "Right", 1);
	...
	for (i = 0; i < flist.num_files; i++) {
		...
		CHARSET_EASY_MODE(f->base, CHARSET_CHAR, CHARSET_CP437, {
			for (j = 0; j < search_pos; j++) {
				if (tolower(out[j]) != tolower(search_str[j])) break;
			}
		});
...
void load_sample_load_page(struct page *page)
{
	...
	create_other(widgets_loadsample + 0, 0, file_list_handle_key, file_list_handle_text_input, file_list_draw);
	...
	sample_speed_pos = 0;
	create_numentry(widgets_loadsample + 2, 64, 14, 7, 1, 3, 9, handle_load_update, 0, 9999999, &sample_speed_pos);

	create_menutoggle(widgets_loadsample + 3, 64, 15, 2, 4, 0, 9, 9, handle_load_update, loop_states);

	sample_loop_beg = 0;
	create_numentry(widgets_loadsample + 4, 64, 16, 7, 3, 5, 9, handle_load_update, 0, 9999999, &sample_loop_beg);
	sample_loop_end = 0;
	create_numentry(widgets_loadsample + 5, 64, 17, 7, 4, 6, 9, handle_load_update, 0, 9999999, &sample_loop_end);

	create_menutoggle(widgets_loadsample + 6, 64, 18, 5, 7, 0, 9, 9, handle_load_update, loop_states);

	sample_susloop_beg = 0;
	create_numentry(widgets_loadsample + 7, 64, 19, 7, 6, 8, 9, handle_load_update, 0, 9999999, &sample_susloop_beg);
	sample_susloop_end = 0;
	create_numentry(widgets_loadsample + 8, 64, 20, 7, 7, 9, 9, handle_load_update, 0, 9999999, &sample_susloop_end);

	create_thumbbar(widgets_loadsample + 9, 63, 33, 9, 8, 10, 0, handle_load_update, 0, 64);
	create_thumbbar(widgets_loadsample + 10, 63, 34, 9, 9, 11, 0, handle_load_update, 0, 64);

	create_thumbbar(widgets_loadsample + 11, 63, 37, 9, 10, 12, 0, handle_load_update, 0, 64);
	create_thumbbar(widgets_loadsample + 12, 63, 38, 9, 11, 13, 0, handle_load_update, 0, 32);
	create_thumbbar(widgets_loadsample + 13, 63, 39, 9, 12, 13, 0, handle_load_update, 0, 255);
