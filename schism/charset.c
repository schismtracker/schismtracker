...
#define DG(ax, eq) \
	{ \
		...
		if ((k1 == c[0] && k2 == c[1]) || (k2 == c[0] && k1 == c[1])) return eq; \
	}
	DG("NB", '#')
	DG("DO", '$')
	DG("At", '@')
...
static int utf8_to_ucs4(const uint8_t *in, uint32_t *out, size_t *size_until_next)
{
	...
	if (c == 0x00) {
		...
		*size_until_next = 1;
		...
		if (!((in[1] ^ 0x80) < 0x40)) return DECODER_ERROR;

		*out = ((uint32_t)(c & 0x1f) << 6) | (uint32_t)(in[1] ^ 0x80);
		...
		*out = ((uint32_t)(c & 0x0f) << 12) | ((uint32_t)(in[1] ^ 0x80) << 6) | (uint32_t)(in[2] ^ 0x80);

		*size_until_next = 3;
	} else if (c < 0xf8) {
		if (!((in[1] ^ 0x80) < 0x40 && (in[2] ^ 0x80) < 0x40 && (in[3] ^ 0x80) < 0x40 && (c >= 0xf1 || in[1] >= 0x90)
			  && (c < 0xf4 || (c == 0xf4 && in[1] < 0x90))))
			return DECODER_ERROR;

		*out = ((uint32_t)(c & 0x07) << 18) | ((uint32_t)(in[1] ^ 0x80) << 12) | ((uint32_t)(in[2] ^ 0x80) << 6)
			   | (uint32_t)(in[3] ^ 0x80);
...
{
	static const uint16_t cp437_table[128] = {/* 0x80 */
...
static int windows1252_to_ucs4(const uint8_t *in, uint32_t *out, size_t *size_until_next)
{
	...
	static const uint16_t windows1252_table[32] = {/* 0x80 */
	...
	if (c >= 0x80 && c < 0xA0) {
		*out = windows1252_table[c - 0x80];
		if (*out == 0xFFFF) return DECODER_ERROR;
	} else {
		*out = c;
	}
...
{
	if (out) *out = char_unicode_to_cp437(ch);
...
static charset_conv_from_ucs4_func conv_from_ucs4_funcs[] = {
	[CHARSET_UTF8] = ucs4_to_utf8,       [CHARSET_UTF16LE] = ucs4_to_utf16LE,
	[CHARSET_UTF16BE] = ucs4_to_utf16BE, [CHARSET_UCS4] = ucs4_do_nothing,
...
CHARSET_VARIATION(internal)
{
	...
	if (!conv_to_ucs4_func || !conv_from_ucs4_func) return CHARSET_ERROR_UNIMPLEMENTED;

	*out = malloc((out_alloc) * sizeof(uint8_t));
	if (!*out) return CHARSET_ERROR_NOMEM;
...
CHARSET_VARIATION(sdl)
{
	static const char *charset_iconv_system_lookup[] = {
		[CHARSET_UTF8] = "UTF-8", [CHARSET_UTF16BE] = "UTF-16BE", [CHARSET_UTF16LE] = "UTF-16LE",
		[CHARSET_UCS4] = "UCS-4",

		[CHARSET_CP437] = "437",

		[CHARSET_CHAR] = "",      [CHARSET_WCHAR_T] = "WCHAR_T"};

	/* A bit hacky, but whatever */
	size_t src_size = strlen((const char *)in);
	size_t out_size = src_size;

	if (src_size <= 0) return CHARSET_ERROR_UNIMPLEMENTED;
	...
	if (cd == (SDL_iconv_t)(-1)) return CHARSET_ERROR_UNIMPLEMENTED;
	...
	*out = malloc(out_size + sizeof(uint32_t));
	...
	while (in_bytes_left) {
		...
		if (old_in_bytes_left == in_bytes_left) break;
	}
...
charset_error_t charset_iconv(const uint8_t *in, uint8_t **out, charset_t inset, charset_t outset)
{
	...
	if (!in) return CHARSET_ERROR_NULLINPUT;

	if (!out) return CHARSET_ERROR_NULLOUTPUT;

	if (inset == outset) return CHARSET_ERROR_INPUTISOUTPUT;

#define TRY_VARIATION(name) \
	state = charset_iconv_##name##_(in, out, inset, outset); \
	if (state == CHARSET_ERROR_SUCCESS || state == CHARSET_ERROR_NOMEM) return state;
...
int charset_strcmp(const uint8_t *in1, charset_t in1set, const uint8_t *in2, charset_t in2set)
{
	...
	if (in1set >= ARRAY_SIZE(conv_to_ucs4_funcs) || in2set >= ARRAY_SIZE(conv_to_ucs4_funcs)) goto charsetfail;

	charset_conv_to_ucs4_func conv1_to_ucs4_func = conv_to_ucs4_funcs[in1set],
							  conv2_to_ucs4_func = conv_to_ucs4_funcs[in2set];

	if (!conv1_to_ucs4_func || !conv2_to_ucs4_func) goto charsetfail;

	for (;;) {
		c1 = conv1_to_ucs4_func(in1 + in1_offset, &codepoint1, &in1_needed);
		c2 = conv1_to_ucs4_func(in2 + in2_offset, &codepoint2, &in2_needed);

		if (c1 == DECODER_ERROR || c2 == DECODER_ERROR) goto charsetfail;

		if (c1 == DECODER_DONE || c2 == DECODER_DONE || codepoint1 != codepoint2) break;
...
int charset_strcasecmp(const uint8_t *in1, charset_t in1set, const uint8_t *in2, charset_t in2set)
{
	...
	for (;;) {
		...
		if (c1 == DECODER_DONE || c2 == DECODER_DONE
			|| charset_simple_case_fold(codepoint1) != charset_simple_case_fold(codepoint2))
			break;
