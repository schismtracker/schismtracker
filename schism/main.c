...
{
	const char *err;
	if (SDL_Init(SDL_INIT_FLAGS) == 0) return;
	err = SDL_GetError();
	if (strstr(err, "mouse")) {
...
static void handle_window_event(SDL_WindowEvent *w)
{
	switch (w->event) {
		case SDL_WINDOWEVENT_SHOWN:
			...
			break;
		case SDL_WINDOWEVENT_HIDDEN: status.flags &= ~IS_VISIBLE; break;
		...
		case SDL_WINDOWEVENT_SIZE_CHANGED: /* tiling window managers */
			video_update();
			/* fallthrough */
		case SDL_WINDOWEVENT_EXPOSED: status.flags |= (NEED_UPDATE); break;
		default:
#if 0
		/* ignored currently */
	...
	while ((opt = getopt_long(argc, argv, SHORT_OPTIONS, long_options, NULL)) != -1) {
		switch (opt) {
			case O_SDL_AUDIODRIVER:
				...
				break;
			case O_SDL_VIDEODRIVER: video_driver = str_dup(optarg); break;
#if USE_NETWORK
			case O_NETWORK: startup_flags |= SF_NETWORK; break;
			case O_NO_NETWORK: startup_flags &= ~SF_NETWORK; break;
#endif
			case O_CLASSIC_MODE:
			...
			case O_NO_FULLSCREEN:
				...
				did_fullscreen = 1;
				break;
			case O_PLAY: startup_flags |= SF_PLAY; break;
			case O_NO_PLAY: startup_flags &= ~SF_PLAY; break;
			case O_FONTEDIT: startup_flags |= SF_FONTEDIT; break;
			case O_NO_FONTEDIT: startup_flags &= ~SF_FONTEDIT; break;
			case O_DISKWRITE: diskwrite_to = optarg; break;
#if ENABLE_HOOKS
			case O_HOOKS: startup_flags |= SF_HOOKS; break;
			case O_NO_HOOKS: startup_flags &= ~SF_HOOKS; break;
#endif
			case O_VERSION:
...
{
	if (ACTIVE_WIDGET.clipboard_paste && ACTIVE_WIDGET.clipboard_paste(e->user.code, e->user.data1)) return;

	if (ACTIVE_PAGE.clipboard_paste && ACTIVE_PAGE.clipboard_paste(e->user.code, e->user.data1)) return;
...
# define _ALTTRACKED_KMOD 0
				...
				case SDL_TEXTINPUT: {
					...
					charset_error_t err =
						charset_iconv((uint8_t *)event.text.text, &input_text, CHARSET_UTF8, CHARSET_CP437);
					if (err || !input_text) {
				...
				case SDL_KEYUP:
					...
					switch (event.key.keysym.sym) {
						case SDLK_NUMLOCKCLEAR: modkey ^= KMOD_NUM; break;
						case SDLK_CAPSLOCK:
						...
						case SDLK_LSHIFT:
						case SDLK_RSHIFT:
							if (event.type == SDL_KEYDOWN) status.flags |= SHIFT_KEY_DOWN;
							else status.flags &= ~SHIFT_KEY_DOWN;
							break;
						default: break;
					};

					if (kk.state == KEY_PRESS) {
						modkey = (event.key.keysym.mod & ~(_ALTTRACKED_KMOD)) | (modkey & _ALTTRACKED_KMOD);
					}
					 ...
					 * the last keydown is the same sym */
						if (last_key == kk.sym) empty_key_repeat();
					} else {
					...
					break;
				case SDL_QUIT: show_exit_prompt(); break;
				case SDL_WINDOWEVENT:
					...
					break;
				case SDL_MOUSEWHEEL: kk.state = -1; /* neither KEY_PRESS nor KEY_RELEASE */
				...
				case SDL_MOUSEBUTTONUP:
					...
					if (event.type != SDL_MOUSEWHEEL) video_translate(event.button.x, event.button.y, &kk.fx, &kk.fy);
					...
					switch (event.button.button) {
						...
						case SDL_BUTTON_LEFT:
							if ((modkey & KMOD_CTRL) || event.button.button == SDL_BUTTON_RIGHT) {
								kk.mouse_button = MOUSE_BUTTON_RIGHT;
							} else if ((modkey & (KMOD_ALT | KMOD_GUI)) || event.button.button == SDL_BUTTON_MIDDLE) {
			 ...
			 * we don't have any dependencies that use SDL anyway. */
				case SCHISM_EVENT_MIDI: midi_engine_handle_event(&event); break;
				...
				case SCHISM_EVENT_PLAYBACK:
					...
					midi_send_flush();
					if (!(status.flags & (DISKWRITER_ACTIVE | DISKWRITER_ACTIVE_PATTERN))) playback_update();
					break;
				...
				case SCHISM_EVENT_NATIVE:
					...
					switch (event.user.code) {
						case SCHISM_EVENT_NATIVE_OPEN: /* open song */ song_load(event.user.data1); break;
						case SCHISM_EVENT_NATIVE_SCRIPT:
							...
							if (charset_strcasecmp(event.user.data1, CHARSET_UTF8, "new", CHARSET_UTF8) == 0) {
								...
								charset_strcasecmp(event.user.data1, CHARSET_UTF8, "save_as", CHARSET_UTF8) == 0) {
								set_page(PAGE_SAVE_MODULE);
							} else if (
								charset_strcasecmp(event.user.data1, CHARSET_UTF8, "export_song", CHARSET_UTF8) == 0) {
								set_page(PAGE_EXPORT_MODULE);
							} else if (
								charset_strcasecmp(event.user.data1, CHARSET_UTF8, "logviewer", CHARSET_UTF8) == 0) {
								set_page(PAGE_LOG);
							} else if (
								charset_strcasecmp(event.user.data1, CHARSET_UTF8, "font_editor", CHARSET_UTF8) == 0) {
								...
								charset_strcasecmp(event.user.data1, CHARSET_UTF8, "pattern", CHARSET_UTF8) == 0) {
								set_page(PAGE_PATTERN_EDITOR);
							} else if (
								charset_strcasecmp(event.user.data1, CHARSET_UTF8, "orders", CHARSET_UTF8) == 0) {
								set_page(PAGE_ORDERLIST_PANNING);
							} else if (
								charset_strcasecmp(event.user.data1, CHARSET_UTF8, "variables", CHARSET_UTF8) == 0) {
								set_page(PAGE_SONG_VARIABLES);
							} else if (
								charset_strcasecmp(event.user.data1, CHARSET_UTF8, "message_edit", CHARSET_UTF8) == 0) {
								...
								charset_strcasecmp(event.user.data1, CHARSET_UTF8, "play_pattern", CHARSET_UTF8) == 0) {
								song_loop_pattern(get_current_pattern(), 0);
							} else if (
								charset_strcasecmp(event.user.data1, CHARSET_UTF8, "play_order", CHARSET_UTF8) == 0) {
								song_start_at_order(get_current_order(), 0);
							} else if (
								charset_strcasecmp(event.user.data1, CHARSET_UTF8, "play_mark", CHARSET_UTF8) == 0) {
								play_song_from_mark();
							} else if (charset_strcasecmp(event.user.data1, CHARSET_UTF8, "stop", CHARSET_UTF8) == 0) {
								song_stop();
							} else if (
								charset_strcasecmp(event.user.data1, CHARSET_UTF8, "calc_length", CHARSET_UTF8) == 0) {
								show_song_length();
							} else if (
								charset_strcasecmp(event.user.data1, CHARSET_UTF8, "sample_page", CHARSET_UTF8) == 0) {
								set_page(PAGE_SAMPLE_LIST);
							} else if (
								charset_strcasecmp(event.user.data1, CHARSET_UTF8, "sample_library", CHARSET_UTF8)
								== 0) {
								set_page(PAGE_LIBRARY_SAMPLE);
							} else if (
								charset_strcasecmp(event.user.data1, CHARSET_UTF8, "init_sound", CHARSET_UTF8) == 0) {
								/* does nothing :) */
							} else if (
								charset_strcasecmp(event.user.data1, CHARSET_UTF8, "inst_page", CHARSET_UTF8) == 0) {
								set_page(PAGE_INSTRUMENT_LIST);
							} else if (
								charset_strcasecmp(event.user.data1, CHARSET_UTF8, "inst_library", CHARSET_UTF8) == 0) {
								set_page(PAGE_LIBRARY_INSTRUMENT);
							} else if (
								charset_strcasecmp(event.user.data1, CHARSET_UTF8, "preferences", CHARSET_UTF8) == 0) {
								set_page(PAGE_PREFERENCES);
							} else if (
								charset_strcasecmp(event.user.data1, CHARSET_UTF8, "system_config", CHARSET_UTF8)
								== 0) {
								set_page(PAGE_CONFIG);
							} else if (
								charset_strcasecmp(event.user.data1, CHARSET_UTF8, "midi_config", CHARSET_UTF8) == 0) {
								set_page(PAGE_MIDI);
							} else if (
								charset_strcasecmp(event.user.data1, CHARSET_UTF8, "palette_page", CHARSET_UTF8) == 0) {
								set_page(PAGE_PALETTE_EDITOR);
							} else if (
								charset_strcasecmp(event.user.data1, CHARSET_UTF8, "fullscreen", CHARSET_UTF8) == 0) {
		...
		int t;
		for (t = 0; t < 10 && !SDL_PollEvent(NULL); t++) SDL_Delay(1);
	}
	schism_exit(0);
}
...
{
#if ENABLE_HOOKS
	if (shutdown_process & EXIT_HOOK) run_exit_hook();
#endif

	if (shutdown_process & EXIT_SAVECFG) cfg_atexit_save();
	...
	if (startup_flags & SF_FONTEDIT) {
		...
		if (song_load_unchecked(initial_song)) {
			if (diskwrite_to) {
				...
				const char *driver =
					(strcasestr(diskwrite_to, ".aif") ? (multi ? "MAIFF" : "AIFF") : (multi ? "MWAV" : "WAV"));
