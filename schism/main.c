...
static void handle_window_event(SDL_WindowEvent *w)
{
	...
	case SDL_WINDOWEVENT_FOCUS_GAINED: video_mousecursor(MOUSE_RESET_STATE); break;
...
static void parse_options(int argc, char **argv)
{
	...
	while ((opt = getopt_long(argc, argv, SHORT_OPTIONS, long_options, NULL)) != -1) {
		switch (opt) {
		case O_SDL_AUDIODRIVER: audio_parse_driver_spec(optarg, (char **)&audio_driver, (char **)&audio_device); break;
...
static void check_update(void)
{
	...
	if (video_is_visible() && (status.flags & NEED_UPDATE)) {
		...
		if (!video_is_focused() && (status.flags & LAZY_REDRAW)) {
			if (!SCHISM_TICKS_PASSED(now, next)) return;
...
# define _ALTTRACKED_KMOD 0
			...
			case SDL_KEYUP:
				...
				if (event.type == SDL_KEYUP) {
					...
					if (last_key == kk.sym) kbd_empty_key_repeat();
				} else {
			...
			case SDL_DROPFILE:
				...
				case PAGE_LIBRARY_INSTRUMENT:
					...
					break;
				default: song_load(event.drop.file); break;
				}
			...
			case SCHISM_EVENT_NATIVE:
				...
				case SCHISM_EVENT_NATIVE_SCRIPT:
					...
					if (charset_strcasecmp(event.user.data1, CHARSET_UTF8, "new", CHARSET_UTF8) == 0) {
						...
						charset_strcasecmp(event.user.data1, CHARSET_UTF8, "sample_library", CHARSET_UTF8) == 0) {
