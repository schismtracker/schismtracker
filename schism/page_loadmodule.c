...
static void set_glob(const char *globspec)
{
	...
	str_realloc(&glob_list_src, globspec, 0);
	...
	if (*selected_widget == 0) {
		for (n = 0; n < flist.num_files; n++) {
			size_t caselen = charset_strncasecmplen(
				flist.files[n]->base, CHARSET_CHAR,
				search_text, CHARSET_CP437, search_text_length);
			if (caselen > max)
				which = n;
		}
		...
		for (n = 0; n < dlist.num_dirs; n++) {
			size_t caselen = charset_strncasecmplen(
				dlist.dirs[n]->base, CHARSET_CHAR,
				search_text, CHARSET_CP437, search_text_length);
			if (caselen > max)
				which = n;
		}
...
static int change_dir(const char *dir)
{
	...
	str_realloc(&cfg_dir_modules, ptr, 0);
