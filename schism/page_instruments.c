...
static void do_exchange_instrument(UNUSED void *data)
{
	...
	if (n < 1 || n > _last_vis_inst())
		return;
	song_exchange_instruments(n, current_instrument);
}
...
static void do_copy_instrument(UNUSED void *data)
{
	...
	song_copy_instrument(n, current_instrument);
}
...
static int _env_handle_key_viewmode(struct key_event *k, song_envelope *env, int *current_node, unsigned int sec)
{
	...
	case SDLK_SPACE:
		...
		song_keydown(-1, current_instrument, last_note, 64,
						KEYDOWN_CHAN_CURRENT, 0);
		return 1;
	case SDLK_RETURN:
		if (!k->state) return 0;
	...
	case SDLK_ESCAPE:
		...
		if (k->mod & (KMOD_ALT)) {
			...
			if (k->state) {
				...
				song_keydown(-1, current_instrument, n, v,
						KEYDOWN_CHAN_CURRENT, 0);
			}
			last_note = n;
		}
