...
static const uint8_t itfmap_chars[] = {
	128, 129, 130, ' ', 128, 129, 141, ' ', 142, 143, 144, ' ', 168, 'C', '-', '0', 131, ' ', 132, ' ', 131, ' ',
	132, ' ', 145, ' ', 146, ' ', 168, 'D', '-', '1', 133, 134, 135, ' ', 140, 134, 135, ' ', 147, 148, 149, ' ',
	168, 'E', '-', '2', ' ', ' ', ' ', ' ', ' ', 139, 134, 138, 153, 148, 152, ' ', 168, 'F', '-', '3', 174, ' ',
	' ', ' ', 155, 132, ' ', 131, 146, ' ', 145, ' ', 168, 'G', '-', '4', 175, ' ', ' ', ' ', 156, 137, 129, 136,
	151, 143, 150, ' ', 168, 'A', '-', '5', 176, ' ', ' ', ' ', 157, ' ', 184, 184, 191, '6', '4', 192, 168, 'B',
	'-', '6', 176, 177, ' ', ' ', 158, 163, 250, 250, 250, 250, 250, ' ', 168, 'C', '#', '7', 176, 178, ' ', ' ',
	159, 164, ' ', ' ', ' ', 185, 186, ' ', 168, 'D', '#', '8', 176, 179, 180, ' ', 160, 165, ' ', ' ', ' ', 189,
	190, ' ', 168, 'E', '#', '9', 176, 179, 181, ' ', 161, 166, ' ', ' ', ' ', 187, 188, ' ', 168, 'F', '#', '1',
	176, 179, 182, ' ', 162, 167, 126, 126, 126, ' ', ' ', ' ', 168, 'G', '#', '2', 154, 154, 154, 154, ' ', ' ',
	205, 205, 205, ' ', 183, ' ', 168, 'A', '#', '3', 169, 170, 171, 172, ' ', ' ', '^', '^', '^', ' ', 173, ' ',
	168, 'B', '#', '4', 193, 194, 195, 196, 197, 198, 199, 200, 201, ' ', ' ', ' ', ' ', ' ', ' ', ' ',
};
...
{
	if (cur_font < 0) cur_font = 0; /* weird! */
	if (cur_font < top_font) top_font = cur_font;
	else if (cur_font > top_font + (VISIBLE_FONTS - 1)) top_font = cur_font - (VISIBLE_FONTS - 1);
}
...
static int fontgrep(dmoz_file_t *f)
{
	...
	if (f->sort_order == -100) return 1;         /* this is our font.cfg, at the top of the list */
	if (f->type & TYPE_BROWSABLE_MASK) return 0; /* we don't care about directories and stuff */
...
static void load_fontlist(void)
{
	...
	if (dmoz_read(font_dir, &flist, NULL, NULL) < 0) log_perror(font_dir);
	free(font_dir);
...
static void draw_frame(const char *name, int x, int y, int inner_width, int inner_height, int active)
{
	...
	if (len > inner_width + 2) len = inner_width + 2;
	...
	for (n = 0; n < len + 1; n++) draw_char(129, x + n + 1, y, c, 2);
...
static inline void draw_editbox(void)
{
	...
	for (i = 0; i < 8; i++) {
		...
		for (j = 0; j < 8; j++) {
			...
			else draw_char(c, INNER_X(EDITBOX_X) + 1 + i, INNER_Y(EDITBOX_Y) + 3 + j, fg, 0);
		}
	}
...
static inline void draw_charmap(void)
{
	...
	if (selected_item == CHARMAP) {
		while (n) {
			n--;
			draw_char(
				n, INNER_X(CHARMAP_X) + n % 16, INNER_Y(CHARMAP_Y) + n / 16, (n == current_char ? 0 : 1),
				(n == current_char ? 3 : 0));
		}
	} else {
		while (n) {
			n--;
			draw_char(n, INNER_X(CHARMAP_X) + n % 16, INNER_Y(CHARMAP_Y) + n / 16, (n == current_char ? 3 : 1), 0);
		}
	}
}
...
static inline void draw_itfmap(void)
{
	...
	if (itfmap_pos < 0 || itfmap_chars[itfmap_pos] != current_char) {
		...
		if (ptr == NULL) itfmap_pos = -1;
		else itfmap_pos = ptr - itfmap_chars;
	}
...
static inline void draw_helptext(void)
{
	...
	for (line = INNER_Y(HELPTEXT_Y); *ptr; line++) {
		...
		if (!eol) eol = (unsigned char *)strchr((char *)ptr, '\0');
		for (column = INNER_X(HELPTEXT_X); ptr < eol; ptr++, column++) draw_char(*ptr, column, line, 12, 0);
		ptr++;
	}
	for (line = 0; line < 10; line++) draw_char(168, INNER_X(HELPTEXT_X) + 43, INNER_Y(HELPTEXT_Y) + line, 12, 0);
	...
	for (line = INNER_Y(HELPTEXT_Y); *ptr; line++) {
		...
		for (column = INNER_X(HELPTEXT_X) + 45; ptr < eol; ptr++, column++) draw_char(*ptr, column, line, 12, 0);
		ptr++;
	}
...
static void handle_mouse(struct key_event *k)
{
	...
	if (POINT_IN_FRAME(x, y, EDITBOX)) {
		selected_item = EDITBOX;
		if (POINT_IN(x, y, EDITBOX)) handle_mouse_editbox(k);
	} else if (POINT_IN_FRAME(x, y, CHARMAP)) {
		selected_item = CHARMAP;
		if (POINT_IN(x, y, CHARMAP)) handle_mouse_charmap(k);
	} else if (POINT_IN_FRAME(x, y, ITFMAP)) {
		selected_item = ITFMAP;
		if (POINT_IN(x, y, ITFMAP)) handle_mouse_itfmap(k);
	} else {
		//printf("stray click\n");
		return;
	}
