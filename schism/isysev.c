...
static void event_loop(void)
{
	...
	while (SDL_WaitEvent(&sdlev)) {
		...
		case SDL_KEYDOWN:
			...
			if (sdlev.key.keysym.mod & KMOD_CTRL)
				ev.bits.modifier |= SKMODE_CTRL;
			if (sdlev.key.keysym.mod & KMOD_ALT)
				ev.bits.modifier |= SKMODE_ALT;
			if (sdlev.key.keysym.mod & KMOD_SHIFT)
				ev.bits.modifier |= SKMODE_SHIFT;
...
int main(int argc, char **argv)
{
	...
	for (jn = 0; jn < n; jn++) {
		...
		printf("Joystick #%d [%s]\n\taxes:%d buttons:%d hats:%d balls:%d\n", jn, SDL_JoystickName(jn),
			SDL_JoystickNumAxes(js), SDL_JoystickNumButtons(js), SDL_JoystickNumHats(js),
