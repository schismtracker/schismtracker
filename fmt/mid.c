...
static void keyvol(midi_config_t *g, unsigned char **pp, struct midi_track *t, int vol)
{
	int j;
	vol = volfix(vol);
	if (vol > 127) vol = 127;
	for (j = 0; j < 128; j++) {
		...
		macro(pp, t->note_on[j]-1,
				g->set_volume, vol, j, vol, t);
	}
}
static void keyoff(midi_config_t *g, unsigned char **pp, struct midi_track *t)
{
	int j;
	for (j = 0; j < 128; j++) {
		if (!t->note_on[j]) continue;
...
int fmt_mid_save_song(disko_t *ds, song_t *song)
{
	song_note_t *nb;
	midi_config_t *g;
	uint8_t *orderlist;
	struct midi_track trk[MAX_CHANNELS];
	struct midi_map map[MAX_SAMPLES], *m;
	...
	if (song->flags & SONG_INSTRUMENTMODE) {
		SCHISM_STATIC_ASSERT(MAX_INSTRUMENTS == MAX_SAMPLES, "This code assumes MAX_INSTRUMENTS and MAX_SAMPLES are equal");
		for (i = 1; i <= MAX_INSTRUMENTS; i++) {
			ins = song->instruments[i];
			if (!ins || csf_instrument_is_empty(ins)) continue;
		...
		for (i = 1; i <= MAX_SAMPLES; i++) {
			smp = song->samples + i;
			if (csf_sample_is_empty(smp)) continue;
...

	{
		...
		disko_write(ds, "MThd", 4);

		dw = bswapLE32(6);
		disko_write(ds, &dw, sizeof(dw));
		w = bswapLE16(1); // multi-track
		disko_write(ds, &w, sizeof(w));
		w = bswapLE16(MAX_CHANNELS); // number of track chunks
		disko_write(ds, &w, sizeof(w));
		w = bswapLE16(0x60);
		disko_write(ds, &w, sizeof(w));
	}
	...
	for (i = 0; i < 64; i++) {
		p = packet;
		macro(&p, 0, g->set_panning, trk[i].pan, 0, 0, trk+i);
	...
	while (order >= 0 && order < MAX_ORDERS && orderlist[order] != 255) {
		...
		if (!(nb = song->patterns[orderlist[order]]) || !(left = song->pattern_size[orderlist[order]])) {
		...
		while (left >= 0) {
			...
			for (j = 0; j < 64; j++) {
				...
				case FX_TEMPO:
					...
					trk[j].old_tempo = nt;
					break;
				case FX_SPECIAL: // FX_S3MCMDEX
					if ((nb[j].param & 0xF0) == 0xe0) {
						pad_all(trk,(nb[j].param & 15),
								tempo,1);
					}
					break;
				};
			...
			for (i = 0; i < 64; i++) {
				...
				if (trk[i].muted) {
					...
					vol = 127;
					if (nb->voleffect == VOLFX_VOLUME)
						vol = nb->volparam * 2;
					if (nb->effect == FX_VOLUME)
						vol = nb->param * 2;
					if (vol > 127) vol = 127;
				...
				} else if (nb->effect == FX_VOLUME) {
					vol = nb->param * 2;
					keyvol(g,&p, trk+i, vol);
				} else if (nb->voleffect == VOLFX_VOLUME) {
					vol = nb->volparam * 2;
					keyvol(g,&p, trk+i, vol);
				}
				j = nb->param;
				...
				case FX_PATTERNBREAK:
					...
					left = 0;
					break;
				case FX_PANNING: // originally CMD_PANNING8, is this correct? --paper
					if (j < 0x80) {
						*p = 0xb0|(m->c-1); p++;
						*p = 0x10; p++;
						*p = (j & 127); p++;
	...
	for (i = 0; i < MAX_CHANNELS; i++) {
		...
		disko_write(ds, (const unsigned char *)"MTrk", 4);
		disko_write(ds, packet, 4);
		if (trk[i].used) {
			disko_write(ds, trk[i].buf, trk[i].used);
			free(trk[i].buf);
		}
	}
