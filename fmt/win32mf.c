...
typedef HRESULT
	WINAPI (*MF_MFGetServiceSpec)(IUnknown *punkObject, REFGUID guidService, REFIID riid, LPVOID *ppvObject);
...
#define SUBTYPE(x, n) \
 do { \
  if (IsEqualGUID(&subtype, &x)) return n; \
 } while (0)
...
#undef SUBTYPE
		...
		log_appendf(
			1, "{%08lX-%04hX-%04hX-%02hhX%02hhX-%02hhX%02hhX%02hhX%02hhX%02hhX%02hhX}", subtype.Data1, subtype.Data2,
			subtype.Data3, subtype.Data4[0], subtype.Data4[1], subtype.Data4[2], subtype.Data4[3], subtype.Data4[4],
			subtype.Data4[5], subtype.Data4[6], subtype.Data4[7]);
#endif
	}
...
static int get_source_reader_information(IMFSourceReader *reader, dmoz_file_t *file)
{
	...
	if (SUCCEEDED(reader->lpVtbl->GetNativeMediaType(
...
static int convert_media_foundation_metadata(IMFMediaSource *source, dmoz_file_t *file)
{
	...
	if (FAILED(source->lpVtbl->CreatePresentationDescriptor(source, &descriptor))) goto cleanup;
	...
	if (FAILED(provider->lpVtbl->GetMFMetadata(provider, descriptor, 0, 0, &metadata))) goto cleanup;

	if (FAILED(metadata->lpVtbl->GetAllPropertyNames(metadata, &propnames))) goto cleanup;
...
fail:
	...
	if (mfreadwrite) FreeLibrary(mfreadwrite);

	if (propsys) FreeLibrary(propsys);
...
{
	if (!media_foundation_initialized) return;
...
\
 if (FAILED(MF_MFCreateSourceResolver(&resolver))) goto cleanup; \
\
 stream = MF_SHCreateMemStream(data, len); \
 if (!stream) goto cleanup; \
\
 if (FAILED(MF_MFCreateMFByteStreamOnStream(stream, &byte_stream))) goto cleanup; \
\
 if (FAILED(resolver->lpVtbl->CreateObjectFromByteStream( \
		 resolver, byte_stream, url, \
...
\
 if (FAILED(unknown_media_source->lpVtbl->QueryInterface( \
		 unknown_media_source, &IID_IMFMediaSource, (void **)&real_media_source))) \
  goto cleanup; \
\
 if (FAILED(MF_MFCreateSourceReaderFromMediaSource(real_media_source, NULL, &reader))) goto cleanup; \
\
 if (FAILED(MF_MFCreateMediaType(&uncompressed_type))) goto cleanup; \
\
 if (FAILED(uncompressed_type->lpVtbl->SetGUID(uncompressed_type, &MF_MT_MAJOR_TYPE, &MFMediaType_Audio))) \
  goto cleanup; \
\
 if (FAILED(uncompressed_type->lpVtbl->SetGUID(uncompressed_type, &MF_MT_SUBTYPE, &MFAudioFormat_PCM))) goto cleanup; \
\
 if (FAILED( \
		 reader->lpVtbl->SetCurrentMediaType(reader, MF_SOURCE_READER_FIRST_AUDIO_STREAM, NULL, uncompressed_type))) \
  goto cleanup; \
\
...
\
 if (FAILED(reader->lpVtbl->GetCurrentMediaType(reader, MF_SOURCE_READER_FIRST_AUDIO_STREAM, &uncompressed_type))) \
  goto cleanup; \
\
 if (FAILED(reader->lpVtbl->SetStreamSelection(reader, MF_SOURCE_READER_FIRST_AUDIO_STREAM, TRUE))) goto cleanup; \
\
...
\
 if (FAILED(uncompressed_type->lpVtbl->GetUINT32(uncompressed_type, &MF_MT_AUDIO_BITS_PER_SAMPLE, &bps))) \
  goto cleanup; \
\
 if (channels <= 0 || channels > 2) goto cleanup; \
\
 if (sps <= 0) goto cleanup; \
\
 if (bps != 8 && bps != 16) goto cleanup; \
\
 flags = SF_LE; \
...
\
 if (reader) reader->lpVtbl->Release(reader); \
\
 if (stream) stream->lpVtbl->Release(stream); \
\
 if (byte_stream) byte_stream->lpVtbl->Release(byte_stream); \
\
 if (uncompressed_type) uncompressed_type->lpVtbl->Release(uncompressed_type);

int fmt_win32mf_read_info(dmoz_file_t *file, const uint8_t *data, size_t length)
{
	if (!media_foundation_initialized) return 0;

	int success = 0;

	wchar_t *url = NULL;
	if (!file->path || charset_iconv(file->path, (uint8_t **)&url, CHARSET_UTF8, CHARSET_WCHAR_T)) url = NULL;
...
{
	if (!reader) return READER_LOAD_ERROR;
	...
	if (FAILED(reader->lpVtbl->ReadSample(
...
cleanup:
	...
	if (buffer) buffer->lpVtbl->Release(buffer);
...
int fmt_win32mf_load_sample(const uint8_t *data, size_t len, song_sample_t *smp)
{
	...
	for (;;) {
		...
		if (loop_success == READER_LOAD_ERROR) goto cleanup;

		if (loop_success == READER_LOAD_DONE) break;

		if (uncompressed_size / channels / (bps / 8) > MAX_SAMPLE_LENGTH) break; /* punt */
	}

	sample_length = uncompressed_size / channels / (bps / 8);
	if (sample_length < 1 || sample_length > MAX_SAMPLE_LENGTH) goto cleanup;
