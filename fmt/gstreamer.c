...
#define SYMBOLS \
	...
	SYMBOL(gobject, gulong, g_signal_connect_data, \
	...
	SYMBOL(gstpbutils, GstDiscovererInfo *, gst_discoverer_discover_uri, \
		(GstDiscoverer * discoverer, const gchar *uri, GError **err)) \
	...
	SYMBOL(gstpbutils, const gchar *, gst_discoverer_stream_info_get_stream_type_nick, \
	...
	SYMBOL(gstreamer, GstMessage *, gst_bus_timed_pop_filtered, \
...
#  undef g_free
#  define g_free(mem) \
	  (__builtin_object_size((mem), 0) != ((size_t)-1)) ? GST_g_free_sized(mem, __builtin_object_size((mem), 0)) \
							    : GST_g_free(mem)
# else
				...
				for (const GList *l = stream_list; l != NULL; l = l->next) {
					...
					if (stream_info && audio_info) {
						const gchar *display_name
							= GST_gst_discoverer_stream_info_get_stream_type_nick(
								stream_info);
						guint sample_rate
							= GST_gst_discoverer_audio_info_get_sample_rate(audio_info);
						...
						if (caps) {
							gchar *decoder_description
								= GST_gst_pb_utils_get_decoder_description(caps);

							if (decoder_description) {
								// gst_pb_utils_get_decoder_description must be freed,
								// but the value pointed at by file->description will
								// never be freed. So, we need to "intern" the string,
								// creating a single canonical copy for the lifetime of
								// the process.
							...
							if (GST_gst_caps_get_size(caps)) {
								static const gchar *RAW_CODEC_NAMES[] = {"audio/x-wav",
									"audio/x-flac", "audio/x-alac", NULL};

								GstStructure *structure
									= GST_gst_caps_get_structure(caps, 0);

								const gchar *codec_name
									= GST_gst_structure_get_name(structure);

								is_raw_audio = GST_g_strv_contains(
									RAW_CODEC_NAMES, codec_name);
							}
						...
						if ((channels >= 1) && (bits >= 8)) {
							...
							file->type |= (is_raw_audio ? TYPE_SAMPLE_PLAIN
										    : TYPE_SAMPLE_COMPR);
...
static void source__need_data(GstElement *source, guint unused_size, gpointer data)
{
	...
	if (num_read == 0) {
		...
		GstBuffer *buffer_obj
			= GST_gst_buffer_new_wrapped(buffer, num_read); /* takes ownership of buffer's lifetime */
...
fail:
	if (audio_caps_matcher)
		GST_gst_caps_unref(audio_caps_matcher);
	if (caps)
		GST_gst_caps_unref(caps);
	if (convert_sink_pad)
		GST_gst_object_unref(convert_sink_pad);
}
...
int fmt_gstreamer_load_sample(slurp_t *fp, song_sample_t *smp)
{
	...
	/* Set up the appsrc */
	GST_g_object_set(G_OBJECT(source), "format", GST_FORMAT_BYTES, "stream-type", GST_APP_STREAM_TYPE_STREAM, NULL);
...
#define ADD_CAPS(FORMAT) \
	do { \
		GST_gst_caps_append_structure(caps, GST_gst_structure_new("audio/x-raw", "format", G_TYPE_STRING, \
							    FORMAT, "channels", G_TYPE_INT, 1, NULL)); \
\
		GST_gst_caps_append_structure( \
			caps, GST_gst_structure_new("audio/x-raw", "format", G_TYPE_STRING, FORMAT, "channels", \
				      G_TYPE_INT, 2, "layout", G_TYPE_STRING, "interleaved", NULL)); \
	} while (0)
...
#undef ADD_CAPS
	...
	GST_g_object_set(G_OBJECT(format), "caps", caps, NULL);

	GST_gst_caps_unref(caps);

	/* Set up the appsink */
	GST_g_object_set(G_OBJECT(sink), "emit-signals", TRUE, "sync", FALSE, NULL);

	GST_g_signal_connect(sink, "new-sample", G_CALLBACK(sink__new_sample), &buffer);

	/* Link up the pipeline */
	GST_gst_bin_add_many(GST_BIN(pipeline), source, decode, convert, format, sink, NULL);
...
int gstreamer_init(void)
{
	...
	/* Initialize GStreamer -- do not call gst_deinit because it isn't subsequently possible to reinitialize in the
	 * same process */
