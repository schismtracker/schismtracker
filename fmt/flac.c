...
on_read(const FLAC__StreamDecoder *decoder, FLAC__byte buffer[], size_t *bytes, void *client_data)
{
	struct flac_file *flac_file = (struct flac_file *)client_data;
...
static FLAC__StreamDecoderWriteStatus on_write(
	const FLAC__StreamDecoder *decoder, const FLAC__Frame *frame, const FLAC__int32 *const buffer[], void *client_data)
{
	struct flac_file *flac_file = (struct flac_file *)client_data;

	/* invalid? */
	if (!flac_file->streaminfo.total_samples || !flac_file->streaminfo.channels || flac_file->streaminfo.channels > 2)
		return FLAC__STREAM_DECODER_WRITE_STATUS_ABORT;

	if (frame->header.number.sample_number == 0) {
		/* allocate our buffer */
		flac_file->uncompressed.len =
			(size_t)(flac_file->streaminfo.total_samples * flac_file->streaminfo.channels * flac_file->streaminfo.bits_per_sample / 8);
		flac_file->uncompressed.data = (uint8_t *)malloc(
			flac_file->uncompressed.len
			* ((flac_file->streaminfo.bits_per_sample == 8) ? sizeof(int8_t) : sizeof(int16_t)));
		if (!flac_file->uncompressed.data) return FLAC__STREAM_DECODER_WRITE_STATUS_ABORT;
	...
	if (flac_file->streaminfo.bits_per_sample <= 8) {
		...
		for (i = 0, j = 0; i < block_size; j++)
			for (c = 0; c < flac_file->streaminfo.channels; c++) buf_ptr[i++] = buffer[c][j] << bit_shift;
...
{
	if (flac_file->compressed.len < 4) return 0;

	if (memcmp(flac_file->compressed.data, "fLaC", 4)) return 0;
	...
	FLAC__StreamDecoderInitStatus initStatus = FLAC__stream_decoder_init_stream(
		decoder, on_read, NULL, NULL, NULL, NULL, on_write, on_meta, on_error, flac_file);

	if (meta_only) {
		if (!FLAC__stream_decoder_process_until_end_of_metadata(decoder)) FLAC_ERROR(0);
	} else {
		if (!FLAC__stream_decoder_process_until_end_of_stream(decoder)) FLAC_ERROR(0);
	}
...
int fmt_flac_load_sample(const uint8_t *data, size_t len, song_sample_t *smp)
{
	...
	if (!flac_load(&flac_file, 0)) return 0;
	...
	if (flac_file.flags.sample_rate) smp->c5speed = flac_file.flags.sample_rate;
...
int fmt_flac_read_info(dmoz_file_t *file, const uint8_t *data, size_t len)
{
	...
	if (!flac_load(&flac_file, 1)) return 0;

	file->smp_flags = 0;

	/* don't even attempt */
	if (flac_file.streaminfo.channels > 2 || !flac_file.streaminfo.total_samples || !flac_file.streaminfo.channels)
		return 0;

	if (flac_file.streaminfo.bits_per_sample > 8) file->smp_flags |= CHN_16BIT;

	if (flac_file.streaminfo.channels == 2) file->smp_flags |= CHN_STEREO;
	...
	if (flac_file.flags.sample_rate) file->smp_speed = flac_file.flags.sample_rate;
...
static FLAC__StreamEncoderWriteStatus write_on_write(
	const FLAC__StreamEncoder *encoder, const FLAC__byte buffer[], size_t bytes, uint32_t samples,
...
write_on_seek(const FLAC__StreamEncoder *encoder, FLAC__uint64 absolute_byte_offset, void *client_data)
{
	disko_t *fp = (disko_t *)client_data;
...
write_on_tell(const FLAC__StreamEncoder *encoder, FLAC__uint64 *absolute_byte_offset, void *client_data)
{
	disko_t *fp = (disko_t *)client_data;

	long b = disko_tell(fp);
	if (b < 0) return FLAC__STREAM_ENCODER_TELL_STATUS_ERROR;
...
static int flac_save_init(disko_t *fp, int bits, int channels, int rate, int estimate_num_samples)
{
	...
	if (!fwd->encoder) return -1;

	if (!FLAC__stream_encoder_set_channels(fwd->encoder, channels)) return -2;

	if (!FLAC__stream_encoder_set_bits_per_sample(fwd->encoder, bits)) return -3;

	if (rate > FLAC__MAX_SAMPLE_RATE) rate = FLAC__MAX_SAMPLE_RATE;
	...
	if (!FLAC__format_sample_rate_is_subset(rate)) FLAC__stream_encoder_set_streamable_subset(fwd->encoder, false);

	if (!FLAC__stream_encoder_set_sample_rate(fwd->encoder, rate)) return -4;

	if (!FLAC__stream_encoder_set_compression_level(fwd->encoder, 5)) return -5;

	if (!FLAC__stream_encoder_set_total_samples_estimate(fwd->encoder, estimate_num_samples)) return -6;

	if (!FLAC__stream_encoder_set_verify(fwd->encoder, false)) return -7;

	FLAC__StreamEncoderInitStatus init_status =
		FLAC__stream_encoder_init_stream(fwd->encoder, write_on_write, write_on_seek, write_on_tell, NULL, fp);
...
{
	if (flac_save_init(fp, bits, channels, rate, 0)) return DW_ERROR;
...
int fmt_flac_export_body(disko_t *fp, const uint8_t *data, size_t length)
{
	...
	for (i = 0; i < length / bytes_per_sample; i++) {
		if (bytes_per_sample == 2) pcm[i] = (FLAC__int32)(((const int16_t *)data)[i]);
		else if (bytes_per_sample == 1) pcm[i] = (FLAC__int32)(((const int8_t *)data)[i]);
		else return DW_ERROR;
	}
...
{
	if (flac_save_init(
			fp, (smp->flags & CHN_16BIT) ? 16 : 8, (smp->flags & CHN_STEREO) ? 2 : 1, smp->c5speed, smp->length))
	...
	if (fmt_flac_export_tail(fp) != DW_OK) return SAVE_INTERNAL_ERROR;
