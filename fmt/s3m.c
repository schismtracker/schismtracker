...
int fmt_s3m_load_song(song_t *song, slurp_t *fp, unsigned int lflags)
{
	...
	if (tmp == bswapLE16(1)) misc &= ~S3M_UNSIGNED; /* signed samples (ancient s3m) */
	...
	if (!song->initial_speed) song->initial_speed = 6;
	...
	if (slurp_getc(fp) != 0xfc) misc &= ~S3M_CHANPAN; /* stored pan values */
	...
	if (!(lflags & LOAD_NOSAMPLES)) {
		for (n = 0, sample = song->samples + 1; n < nsmp; n++, sample++) {
			if (!sample->length || (sample->flags & CHN_ADLIB)) continue;
	...
	if (gus_addresses > 1) song->mixing_volume = 48;
				...
				if (mask & 128) {
					...
					if (note->effect == FX_SPECIAL) {
						...
						if (note->param == 0xd0) {
							...
							if (gus_addresses > 1 || ctype >= 0x10) note->effect = FX_NONE;
	...
	if (trkvers == 0x1320) {
		if (!memcmp(reserved, "SCLUB2.0", 8)) {
			...
			special == 0 && uc == 0 && (flags & ~0x50) == 0 && misc == (S3M_UNSIGNED | S3M_CHANPAN)
			&& (nord % 16) == 0) {
			/* from OpenMPT:
			 * MPT 1.0 alpha5 doesn't set the stereo flag, but MPT 1.0 alpha6 does. */

			tid = ((mix_volume & 0x80) != 0) ? "ModPlug Tracker / OpenMPT 1.17" : "ModPlug Tracker 1.0 alpha";
		} else if (special == 0 && uc == 0 && flags == 0 && misc == S3M_UNSIGNED) {
			if (song->initial_global_volume == 128 && mix_volume == 48) tid = "PlayerPRO";
			else // Always stereo
				tid = "Velvet Studio";
	...
	if (!tid) {
		...
		case 0:
			...
			break;
		case 1:
			if (gus_addresses > 1) tid = "Scream Tracker %" PRIu8 ".%02" PRIx8 " (GUS)";
				...
				if (trkvers == 0x1301 && uc == 0) {
					if (!(flags & ~0x50) && (mix_volume & 0x80) && (misc & S3M_CHANPAN))
						...
						!flags && song->initial_global_volume == 96 && mix_volume == 176 && song->initial_tempo == 150
						&& !(misc & S3M_CHANPAN))
						strcpy(song->tracker_id, "deMODifier"); // SoundSmith to S3M converter
					else if (
						!flags && song->initial_global_volume == 128 && song->initial_speed == 6
						&& song->initial_tempo == 125 && !(misc & S3M_CHANPAN))
		...
		case 2:
			...
			else tid = "Imago Orpheus %" PRIu8 ".%02" PRIx8;
			break;
		case 3:
			if (trkvers <= 0x3214) {
		...
		case 5:
			...
			if ((trkvers >> 8) == 0x57) tid = "NESMusa %" PRIu8 ".%" PRIX8; /* tool by Bisquit */
			else if (!reserved16 && uc == 16 && channel_types[1] != 1) tid = "Liquid Tracker %" PRIu8 ".%" PRIX8;
			else if (trkvers == 0x5447) strcpy(song->tracker_id, "Graoumf Tracker");
			else if (trkvers >= 0x5129 && reserved16)
				sprintf(
					song->tracker_id, "OpenMPT %" PRIu8 ".%02" PRIX8 ".%02" PRIX8 ".%02" PRIX8,
					(uint8_t)((trkvers & 0xf00) >> 8), (uint8_t)(trkvers & 0xff), reserved[1], reserved[0]);
			else tid = "OpenMPT %" PRIu8 ".%02" PRIX8;
			break;
	...
	if (tid) sprintf(song->tracker_id, tid, (uint8_t)((trkvers & 0xf00) >> 8), (uint8_t)(trkvers & 0xff));
...
static void write_s3i_header(disko_t *fp, song_sample_t *smp, uint32_t sdata)
{
	...
	for (n = 25; n >= 0; n--)
		if ((smp->name[n] ? smp->name[n] : 32) != 32) break;
	for (; n >= 0; n--) hdr.name[n] = smp->name[n] ? smp->name[n] : 32;
...
static int write_s3m_pattern(disko_t *fp, song_t *song, int pat, uint8_t *chantypes, uint16_t *para_pat)
{
	...
	for (row = 0; row < rows; row++) {
		for (chan = 0; chan < 32; chan++, note++) {
			...
			if (out.instrument != 0) {
				if (song->samples[out.instrument].flags & CHN_ADLIB) type = S3I_TYPE_ADMEL;
				else if (song->samples[out.instrument].data != NULL) type = S3I_TYPE_PCM;
				else type = S3I_TYPE_NONE;
...
int fmt_s3m_save_song(disko_t *fp, song_t *song)
{
	...
	if (!(song->flags & SONG_NOSTEREO)) hdr.mv |= 128;
	...
	*/
	...
	for (n = 0; n < npat; n++) warn |= write_s3m_pattern(fp, song, n, chantypes, para_pat);
	...
	for (n = 0, smp = song->samples + 1; n < nsmp; n++, smp++) {
		...
		csf_write_sample(
			fp, smp,
			SF_LE | SF_PCMU | ((smp->flags & CHN_16BIT) ? SF_16 : SF_8) | ((smp->flags & CHN_STEREO) ? SF_SS : SF_M),
			UINT32_MAX);
	}
	...
	for (n = 0; n < 32; n++) {
		...
		if (ch->volume != 64) warn |= 1 << WARN_CHANNELVOL;

		//mphack: channel panning range
		b = ((chantypes[n] & 0x7f) < 0x20) ? ((ch->panning * 15) / 64) : 0;
		disko_putc(fp, b);
	}
	...
	for (n = 0; n < MAX_WARN; n++) {
		if (warn & (1 << n)) log_appendf(4, " Warning: %s unsupported in S3M format", s3m_warnings[n]);
	}
