...
int fmt_pat_read_info(dmoz_file_t *file, const uint8_t *data, size_t length)
{
	...
	if ((length <= sizeof(struct GF1PatchHeader)) || (memcmp(header->sig, "GF1PATCH", 8) != 0)
...
int fmt_pat_load_instrument(const uint8_t *data, size_t length, int slot)
{
	...
	for (i = 0; i < nsamp; i++) {
		...
		if (gfsamp.smpmode & 32) {
			if (gfsamp.smpmode & 4) smp->flags |= CHN_SUSTAINLOOP;
			if (gfsamp.smpmode & 8) smp->flags |= CHN_PINGPONGSUSTAIN;
		} else {
