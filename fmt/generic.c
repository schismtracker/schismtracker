...
static int _mod_period_to_note(int period)
{
	...
	if (period)
		for (n = 0; n <= NOTE_LAST; n++)
			if (period >= (32 * period_table[n % 12] >> (n / 12 + 2))) return n + 1;
	return NOTE_NONE;
}
...
int convert_voleffect(uint8_t *e, uint8_t *p, int force)
{
	...
	case FX_VIBRATO: {
		...
		/* can't do this */
		if (speed && depth && !force) return 0;

		if (speed) {
			if (force) speed = MIN(speed, 9);
			else if (speed > 9) return 0;
			...
			*p = speed;
			...
			if (force) depth = MIN(depth, 9);
			else if (depth > 9) return 0;
	...
	case FX_VOLUMESLIDE:
		...
		if (*p == 0) return 0;
		if ((*p & 0xf) == 0) { // Dx0 / Cx
			if (force) *p = MIN(*p >> 4, 9);
			else if ((*p >> 4) > 9) return 0;
			else *p >>= 4;
			*e = VOLFX_VOLSLIDEUP;
...
void handle_stm_effects(song_note_t *chan_note)
{
	...
	case FX_VOLUMESLIDE:
		...
		if (chan_note->param & 0x0f && chan_note->param >> 4) chan_note->param &= 0x0f;

	case FX_PORTAMENTODOWN:
	case FX_PORTAMENTOUP:
		if (!chan_note->param) chan_note->effect = FX_NONE;
		break;
	case FX_PATTERNBREAK: chan_note->param = (chan_note->param & 0xf0) * 10 + (chan_note->param & 0xf); break;
