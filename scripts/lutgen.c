#define compile exec gcc - Wall - pedantic - std = gnu99 - lm lutgen.c - o lutgen || exit 255
...
#ifdef SPLINE_CLAMPFORUNITY
		sum = cubic_spline_lut[indx + 0] + cubic_spline_lut[indx + 1] + cubic_spline_lut[indx + 2]
...
void windowed_fir_init(void)
{
	...
	for (pcl = 0; pcl < WFIR_LUTLEN; pcl++) {
		...
		for (cc = 0; cc < WFIR_WIDTH; cc++) {
			...
			windowed_fir_lut[indx + cc] = (signed short)((coef < -scale) ? -scale : ((coef > scale) ? scale : coef));
		}
	}
}
