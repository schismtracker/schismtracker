...
GCC_COMPILE_PPC="gcc-3.3 -arch ppc -DREALLY_BIG_ENDIAN \
...
if test x$link = xyes; then
    GCC_COMPILE_PPC=`echo "$GCC_COMPILE_PPC $GCC_LINK_PPC" | sed -e 's,^gcc,g++,'`
    GCC_COMPILE_X86=`echo "$GCC_COMPILE_X86 $GCC_LINK_X86" | sed -e 's,^gcc,g++,'`
fi
...
echo "+pcc $GCC_COMPILE_PPC $ppc_args"
