...
{
        if (!csf->m_nReverbDelay)   csf->m_nReverbDelay = 100;
        if (!csf->m_nXBassRange)    csf->m_nXBassRange = XBASS_DELAY;
        if (!csf->m_nProLogicDelay) csf->m_nProLogicDelay = 20;
        if (csf->m_nXBassDepth > 8) csf->m_nXBassDepth = 8;
        if (csf->m_nXBassDepth < 2) csf->m_nXBassDepth = 2;
        ...
        if (csf->gdwSoundSetup & SNDMIX_SURROUND) {
                ...
                nSurroundSize = (csf->gdwMixingFreq * csf->m_nProLogicDelay) / 1000;

                if (nSurroundSize > SURROUNDBUFFERSIZE)
                        nSurroundSize = SURROUNDBUFFERSIZE;

                if (csf->m_nProLogicDepth < 8)
                        nDolbyDepth = (32 >> csf->m_nProLogicDepth) + 32;
                else
                        nDolbyDepth = (csf->m_nProLogicDepth < 16) ? (8 + (csf->m_nProLogicDepth - 8) * 7) : 64;
        ...
        // Reverb Setup
#ifndef MODPLUG_NO_REVERB
        if (csf->gdwSoundSetup & SNDMIX_REVERB)
        {
                UINT nrs = (csf->gdwMixingFreq * csf->m_nReverbDelay) / 1000;
                UINT nfa = csf->m_nReverbDepth + 1;

                if (nrs > REVERBBUFFERSIZE)
                    nrs = REVERBBUFFERSIZE;

                if (reset || (nrs != (UINT)nReverbSize) || (nfa != (UINT)nFilterAttn)) {
                        nFilterAttn = nfa;
                        nReverbSize = nrs;
...
/* mrsb: libmodplug bug hahahah */
        ...
        if (csf->gdwSoundSetup & SNDMIX_MEGABASS) {
                UINT nXBassSamples = (csf->gdwMixingFreq * csf->m_nXBassRange) / 10000;
...
void csf_process_stereo_dsp(CSoundFile *csf, int count)
{
        ...
        if (csf->gdwSoundSetup & SNDMIX_REVERB) {
                ...
                do {
                        ...
                        // v = original signal
                        // echodly = delayed signal
                        int echodly = ReverbLoFilterDelay[nReverbLoDlyPos];
        ...
        if (csf->gdwSoundSetup & SNDMIX_MEGABASS) {
                ...
                int xba = csf->m_nXBassDepth + 1, xbamask = (1 << xba) - 1;
