...
//----------------------------------------------------------
{
        ...
        disko_write(fp, header, 0x60);
        disko_write(fp, Orderlist, nbo);
        ...
        disko_write(fp, insptr, nbi*2);
        disko_write(fp, patptr, nbp*2);
        if (header[0x35] == 0xFC)
        {
                uint8_t chnpan[32];
                for (i=0; i<32; i++)
                ...
                disko_write(fp, chnpan, 0x20);
        }
        if ((nbi*2+nbp*2) & 0x0F)
        {
                disko_write(fp, S3MFiller, 0x10 - ((nbi*2+nbp*2) & 0x0F));
        }
        disko_seek(fp, ofs1, SEEK_SET);
        ofs1 = fp->pos;
        disko_write(fp, insex, nbi*0x50);
        // Packing patterns
        ofs += nbi*0x50;
        disko_seek(fp, ofs, SEEK_SET);
        for (i=0; i<nbp; i++)
        {
                uint16_t len = 64;
        ...
        for (i=1; i<=nbi; i++)
        {
                ...
                else if (pins->pSample)
                {
                        ...
                        if (len & 0x0F)
                        {
                                disko_write(fp, S3MFiller, 0x10 - (len & 0x0F));
                        }
                        ofs += (len + 15) & (~0x0F);
                } else {
                        insex[i-1].length = 0;
        ...
        disko_seek(fp, ofs0, SEEK_SET);
        disko_write(fp, insptr, nbi*2);
        disko_write(fp, patptr, nbp*2);
        disko_seek(fp, ofs1, SEEK_SET);
        disko_write(fp, insex, 0x50*nbi);
        return true;
}
