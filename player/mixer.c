...
#define SNDMIX_GETMONOVOL16NOIDO int vol = p[position >> 16];
...
#define SNDMIX_GETMONOVOL8SPLINE \
	...
	int vol = (cubic_spline_lut[poslo] * (int)p[poshi - 1] + cubic_spline_lut[poslo + 1] * (int)p[poshi] \
			   + cubic_spline_lut[poslo + 3] * (int)p[poshi + 2] + cubic_spline_lut[poslo + 2] * (int)p[poshi + 1]) \
			  >> SPLINE_8SHIFT;
...
#define SNDMIX_GETSTEREOVOL8SPLINE \
	...
	int vol_l = (cubic_spline_lut[poslo] * (int)p[(poshi - 1) * 2] + cubic_spline_lut[poslo + 1] * (int)p[(poshi) * 2] \
				 ...
				 + cubic_spline_lut[poslo + 3] * (int)p[(poshi + 2) * 2]) \
				>> SPLINE_8SHIFT; \
	int vol_r = \
		(cubic_spline_lut[poslo] * (int)p[(poshi - 1) * 2 + 1] + cubic_spline_lut[poslo + 1] * (int)p[(poshi) * 2 + 1] \
		 + cubic_spline_lut[poslo + 2] * (int)p[(poshi + 1) * 2 + 1] \
		 + cubic_spline_lut[poslo + 3] * (int)p[(poshi + 2) * 2 + 1]) \
		>> SPLINE_8SHIFT;
...
#define SNDMIX_PROCESSFILTER \
	ta = (MUL_32_TO_64(vol, chan->filter_a0) + MUL_32_TO_64(FILT_CLIP(fy1), chan->filter_b0) \
		  + MUL_32_TO_64(FILT_CLIP(fy2), chan->filter_b1) + (1 << (FILTERPRECISION - 1))) \
		 >> FILTERPRECISION; \
	fy2 = fy1; \
	fy1 = ta; \
	vol = ta;
...
#define SNDMIX_PROCESSSTEREOFILTER \
	ta = (MUL_32_TO_64(vol_l, chan->filter_a0) + MUL_32_TO_64(FILT_CLIP(fy1), chan->filter_b0) \
		  + MUL_32_TO_64(FILT_CLIP(fy2), chan->filter_b1) + (1 << (FILTERPRECISION - 1))) \
		 >> FILTERPRECISION; \
	tb = (MUL_32_TO_64(vol_r, chan->filter_a0) + MUL_32_TO_64(FILT_CLIP(fy3), chan->filter_b0) \
		  + MUL_32_TO_64(FILT_CLIP(fy4), chan->filter_b1) + (1 << (FILTERPRECISION - 1))) \
		 >> FILTERPRECISION; \
	fy2 = fy1; \
	fy1 = ta; \
	vol_l = ta; \
...
vol >>=
	(WFIR_16BITSHIFT
	 - WFIR_8SHIFT); //This is used to compensate, since the code assumes that it always outputs to 16bits
...
static const mix_interface_t mix_functions[2 * 2 * 16] = {
	// No SRC
	Mono8BitMix, Mono16BitMix, Stereo8BitMix, Stereo16BitMix, Mono8BitRampMix, Mono16BitRampMix, Stereo8BitRampMix,
	Stereo16BitRampMix,

	// No SRC, Filter
	FilterMono8BitMix, FilterMono16BitMix, FilterStereo8BitMix, FilterStereo16BitMix, FilterMono8BitRampMix,
	FilterMono16BitRampMix, FilterStereo8BitRampMix, FilterStereo16BitRampMix,

	// Linear SRC
	Mono8BitLinearMix, Mono16BitLinearMix, Stereo8BitLinearMix, Stereo16BitLinearMix, Mono8BitLinearRampMix,
	Mono16BitLinearRampMix, Stereo8BitLinearRampMix, Stereo16BitLinearRampMix,

	// Linear SRC, Filter
	FilterMono8BitLinearMix, FilterMono16BitLinearMix, FilterStereo8BitLinearMix, FilterStereo16BitLinearMix,
	FilterMono8BitLinearRampMix, FilterMono16BitLinearRampMix, FilterStereo8BitLinearRampMix,
	FilterStereo16BitLinearRampMix,

	// Spline SRC
	Mono8BitSplineMix, Mono16BitSplineMix, Stereo8BitSplineMix, Stereo16BitSplineMix, Mono8BitSplineRampMix,
	Mono16BitSplineRampMix, Stereo8BitSplineRampMix, Stereo16BitSplineRampMix,

	// Spline SRC, Filter
	FilterMono8BitSplineMix, FilterMono16BitSplineMix, FilterStereo8BitSplineMix, FilterStereo16BitSplineMix,
	FilterMono8BitSplineRampMix, FilterMono16BitSplineRampMix, FilterStereo8BitSplineRampMix,
	FilterStereo16BitSplineRampMix,
	...
	FilterMono8BitFirFilterMix, FilterMono16BitFirFilterMix, FilterStereo8BitFirFilterMix,
	FilterStereo16BitFirFilterMix, FilterMono8BitFirFilterRampMix, FilterMono16BitFirFilterRampMix,
...
static const mix_interface_t fastmix_functions[2 * 2 * 16] = {
	...
	FastMono8BitLinearRampMix,
	FastMono16BitLinearRampMix,
	Stereo8BitLinearRampMix,
	Stereo16BitLinearRampMix,

	// Linear SRC, Filter
	FilterMono8BitLinearMix,
	FilterMono16BitLinearMix,
	FilterStereo8BitLinearMix,
	FilterStereo16BitLinearMix,
	FilterMono8BitLinearRampMix,
	FilterMono16BitLinearRampMix,
	FilterStereo8BitLinearRampMix,
	FilterStereo16BitLinearRampMix,

	// Spline SRC
	FastMono8BitSplineMix,
	FastMono16BitSplineMix,
	Stereo8BitSplineMix,
	Stereo16BitSplineMix,
	FastMono8BitSplineRampMix,
	FastMono16BitSplineRampMix,
	Stereo8BitSplineRampMix,
	Stereo16BitSplineRampMix,

	// Spline SRC, Filter
	FilterMono8BitSplineMix,
	FilterMono16BitSplineMix,
	FilterStereo8BitSplineMix,
	FilterStereo16BitSplineMix,
	FilterMono8BitSplineRampMix,
	FilterMono16BitSplineRampMix,
	FilterStereo8BitSplineRampMix,
	FilterStereo16BitSplineRampMix,

	// FirFilter SRC
	FastMono8BitFirFilterMix,
	FastMono16BitFirFilterMix,
	Stereo8BitFirFilterMix,
	Stereo16BitFirFilterMix,
	FastMono8BitFirFilterRampMix,
	FastMono16BitFirFilterRampMix,
	Stereo8BitFirFilterRampMix,
	Stereo16BitFirFilterRampMix,

	// FirFilter SRC, Filter
	FilterMono8BitFirFilterMix,
	FilterMono16BitFirFilterMix,
	FilterStereo8BitFirFilterMix,
	FilterStereo16BitFirFilterMix,
	FilterMono8BitFirFilterRampMix,
	FilterMono16BitFirFilterRampMix,
