	...
	switch (flags) {
		...
		case RS_IT21516S:
			...
			if (flags == RS_IT2148S || flags == RS_IT2158S) {
				uint32_t offset =
					it_decompress8(sample->data, sample->length, buffer, memsize, (flags == RS_IT2158S), 2);
				it_decompress8(
					sample->data + 1, sample->length, buffer + offset, memsize - offset, (flags == RS_IT2158S), 2);
			} else {
				uint32_t offset =
					it_decompress16(sample->data, sample->length, buffer, memsize, (flags == RS_IT21516S), 2);
				it_decompress16(
