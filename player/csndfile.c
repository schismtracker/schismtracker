...
{
	if (!csf->patterns[n]) return 1;
	if (csf->pattern_size[n] != 64) return 0;
...
{
	return (
		smp->data == NULL && name_is_blank(smp->name) && smp->filename[0] == '\0' && smp->c5speed == 8363
		&& smp->volume == 64 * 4 //mphack
		&& smp->global_volume == 64 && smp->panning == 0 && !(smp->flags & (CHN_LOOP | CHN_SUSTAINLOOP | CHN_PANNING))
		&& smp->length == 0 && smp->loop_start == 0 && smp->loop_end == 0 && smp->sustain_start == 0
		&& smp->sustain_end == 0 && smp->vib_type == VIB_SINE && smp->vib_rate == 0 && smp->vib_depth == 0
		&& smp->vib_speed == 0);
}
...
{
	return (
		env->nodes == 2 && env->loop_start == 0 && env->loop_end == 0 && env->sustain_start == 0
...
int csf_instrument_is_empty(song_instrument_t *ins)
{
	...
	for (n = 0; n < NOTE_LAST - NOTE_FIRST; n++) {
		if (ins->sample_map[n] != 0 || ins->note_map[n] != (n + NOTE_FIRST)) return 0;
	}
	return (
		name_is_blank(ins->name) && ins->filename[0] == '\0'
		&& ins->flags == 0 /* No envelopes, loop points, panning, or carry flags set */
		&& ins->nna == NNA_NOTECUT && ins->dct == DCT_NONE && ins->dca == DCA_NOTECUT && env_is_blank(&ins->vol_env, 64)
		&& ins->global_volume == 128 && ins->fadeout == 0 && ins->vol_swing == 0 && env_is_blank(&ins->pan_env, 32)
		&& ins->panning == 32 * 4      //mphack
		&& ins->pitch_pan_center == 60 // C-5 (blah)
		&& ins->pitch_pan_separation == 0 && ins->pan_swing == 0 && env_is_blank(&ins->pitch_env, 32) && ins->ifc == 0
		&& ins->ifr == 0 && ins->midi_channel_mask == 0 && ins->midi_program == -1 && ins->midi_bank == -1);
}
...
{
	int n = 0;
	while (n < MAX_ORDERS && csf->orderlist[n] != ORDER_LAST) n++;
	return n ? n - 1 : 0;
}
...
int csf_get_num_patterns(song_t *csf)
{
	...
	while (n && csf_pattern_is_empty(csf, n)) n--;
	return n + 1;
}
...
{
	int n = MAX_SAMPLES - 1;
	while (n > 0 && csf_sample_is_empty(csf->samples + n)) n--;
	return n;
}
...
int csf_get_num_instruments(song_t *csf)
{
	...
	while (n > 0 && csf_instrument_is_empty(csf->instruments[n])) n--;
	return n;
}
...
int csf_first_blank_sample(song_t *csf, int start)
{
	...
	for (n = MAX(start, 1); n < MAX_SAMPLES; n++) {
		if (csf_sample_is_empty(csf->samples + n)) return n;
	}
	return -1;
}
...
int csf_first_blank_instrument(song_t *csf, int start)
{
	...
	for (n = MAX(start, 1); n < MAX_INSTRUMENTS; n++) {
		if (csf_instrument_is_empty(csf->instruments[n])) return n;
	}
	return -1;
}
	...
	for (ipat = 0; ipat < MAX_PATTERNS; ipat++) {
		...
		for (j = 0; j < jmax; j++, p++) {
			if (NOTE_IS_NOTE(p->note)) {
				if ((j % MAX_CHANNELS) > highchan) highchan = j % MAX_CHANNELS;
			}
		}
	}
...
{
	int reset = ((csf->mix_frequency != rate) || (csf->mix_bits_per_sample != bits) || (csf->mix_channels != channels));
...
void csf_set_current_order(song_t *csf, uint32_t position)
{
	...
	if (position > MAX_ORDERS) position = 0;
	if (!position) set_current_pos_0(csf);
...
void csf_reset_playmarks(song_t *csf)
{
	...
	for (n = 1; n < MAX_INSTRUMENTS; n++) {
		if (csf->instruments[n]) csf->instruments[n]->played = 0;
	}
}
...
void csf_loop_pattern(song_t *csf, int pat, int row)
{
	if (pat < 0 || pat >= MAX_PATTERNS || !csf->patterns[pat]) {
		...
		if (row < 0 || row >= csf->pattern_size[pat]) row = 0;
...
uint32_t csf_write_sample(disko_t *fp, song_sample_t *sample, uint32_t flags, uint32_t maxlengthmask)
{
	...
	if (maxlengthmask != UINT32_MAX) len = len > maxlengthmask ? maxlengthmask : (len & maxlengthmask);
	...
	case SF_SI:
		if (!(sample->flags & CHN_STEREO)) SF_FAIL("channel mask", flags & SF_CHN_MASK);
		len *= 2;
		break;
	case SF_SS:
		if (!(sample->flags & CHN_STEREO)) SF_FAIL("channel mask", flags & SF_CHN_MASK);
...
#if WORDS_BIGENDIAN
	case SF_LE: byteswap = 1; break;
	case SF_BE: break;
#else
	case SF_LE: break;
	case SF_BE: byteswap = 1; break;
#endif
	default: SF_FAIL("endianness", flags & SF_END_MASK);
	}

	switch (flags & SF_ENC_MASK) {
	case SF_PCMU: add = ((flags & SF_BIT_MASK) == SF_16) ? 32768 : 128; break;
	case SF_PCMS: break;
	case SF_PCMD:
		if ((flags & SF_CHN_MASK) == SF_SS || (flags & SF_CHN_MASK) == SF_M) break;
		/* fallthrough */
	...
	if (!sample || sample->length < 1 || sample->length > MAX_SAMPLE_LENGTH || !sample->data) return 0;
...
uint32_t csf_read_sample(song_sample_t *sample, uint32_t flags, const void *filedata, uint32_t memsize)
{
	...
	switch (flags & SF_CHN_MASK) {
	case SF_SI:
	case SF_SS: mem *= 2; sample->flags |= CHN_STEREO;
	}
	...
	case RS_STPCM16M:
		...
		if (len * 2 <= memsize) {
			...
			for (uint32_t j = 0; j < len; j += 2) {
				...
				*((unsigned short *)(data + j * 2)) = bswapBE16(*((unsigned short *)(src + j)));
				*((unsigned short *)(data + j * 2 + 2)) = bswapBE16(*((unsigned short *)(src + j + len)));
			}
			len *= 2;
		}
		break;
	...
	case RS_IT21516:
		...
		if (flags == RS_IT2148 || flags == RS_IT2158) {
			it_decompress8(sample->data, sample->length, buffer, memsize, (flags == RS_IT2158), 1);
		} else {
			it_decompress16(sample->data, sample->length, buffer, memsize, (flags == RS_IT21516), 1);
		}
		break;
	case RS_IT2148S:
	case RS_IT21416S:
	...
	case RS_IT21516S:
		...
		if (flags == RS_IT2148S || flags == RS_IT2158S) {
			uint32_t offset = it_decompress8(sample->data, sample->length, buffer, memsize, (flags == RS_IT2158S), 2);
			it_decompress8(
				sample->data + 1, sample->length, buffer + offset, memsize - offset, (flags == RS_IT2158S), 2);
		} else {
			uint32_t offset = it_decompress16(sample->data, sample->length, buffer, memsize, (flags == RS_IT21516S), 2);
			it_decompress16(
				sample->data + 2, sample->length, buffer + offset, memsize - offset, (flags == RS_IT21516S), 2);
		}
		break;
	...
	case RS_MDL16:
		if (memsize >= 8) {
			...
			for (uint32_t j = 0; j < sample->length; j++) {
				uint8_t hibyte;
				uint8_t sign;
				if (flags == RS_MDL16) lowbyte = (uint8_t)mdl_read_bits(&bitbuf, &bitnum, &ibuf, 8);
	...
	case SF(7, M, LE, PCMS):
		...
		for (uint32_t j = 0; j < len; j++) sample->data[j] = CLAMP(buffer[j] * 2, -128, 127);
		break;
...
void csf_adjust_sample_loop(song_sample_t *sample)
{
	...
	if (sample->flags & CHN_16BIT) {
		...
		if (sample->flags & CHN_STEREO) {
			data[len * 2 + 6] = data[len * 2 + 4] = data[len * 2 + 2] = data[len * 2] = data[len * 2 - 2];
			data[len * 2 + 7] = data[len * 2 + 5] = data[len * 2 + 3] = data[len * 2 + 1] = data[len * 2 - 1];
		} else {
			data[len + 4] = data[len + 3] = data[len + 2] = data[len + 1] = data[len] = data[len - 1];
		}
	} else {
		signed char *data = sample->data;
		// Adjust end of sample
...
void csf_import_mod_effect(song_note_t *m, int from_xm)
{
	...
	if (!param || (effect == 0x0E && !(param & 0xF))) {
		...
		case 0x0E:
			...
			case 0xB0:
				if (from_xm) break;
			case 0x90: effect = param = 0; break;
			}
			break;
		}
	}
	...
	case 0x0C:
		if (from_xm) {
			...
			m->volparam = param;
			if (m->voleffect > 64) m->voleffect = 64;
			effect = param = 0;
		}
		break;
	case 0x0D:
	...
	case 'P' - 55:
		...
		if (param & 0xF0) param >>= 4;
		else param = (param & 0xf) << 4;
		break;
...
void csf_import_s3m_effect(song_note_t *m, int from_it)
{
	...
	case 'C':
		effect = FX_PATTERNBREAK;
		if (!from_it) param = (param >> 4) * 10 + (param & 0x0F);
		break;
...
void csf_export_s3m_effect(uint8_t *pcmd, uint8_t *pprm, int to_it)
{
	...
	case FX_PATTERNBREAK:
		effect = 'C';
		if (!to_it) param = ((param / 10) << 4) + (param % 10);
		break;
...
void csf_insert_restart_pos(song_t *csf, uint32_t restart_order)
{
	...
	for (max = ord = n = 0; n < MAX_ORDERS && csf->orderlist[n] < MAX_PATTERNS; ord = n, n++)
		if (csf->orderlist[n] > max) max = csf->orderlist[n];
	newpat = max + 1;
	pat = csf->orderlist[ord];
	if (pat >= MAX_PATTERNS || !csf->patterns[pat] || !csf->pattern_size[pat]) return;
	for (max = n, used = 0, n = 0; n < max; n++)
		if (csf->orderlist[n] == pat) used++;

	if (used > 1) {
		// copy the pattern so we don't screw up the playback elsewhere
		while (newpat < MAX_PATTERNS && csf->patterns[newpat]) newpat++;
		if (newpat >= MAX_PATTERNS) return; // no more patterns? sux
		...
		memcpy(csf->patterns[newpat], csf->patterns[pat], sizeof(song_note_t) * MAX_CHANNELS * csf->pattern_size[pat]);
	...
	for (row = 0; row <= max; row++) {
		...
		for (n = 0; n < MAX_CHANNELS; n++, note++) {
			switch (note->effect) {
			case FX_POSITIONJUMP: has_jump = 1; break;
			case FX_PATTERNBREAK:
				has_break = 1;
				if (!note->param) empty = note; // always rewrite C00 with Bxx (it's cleaner)
				break;
			case FX_NONE:
				if (!empty) empty = note;
				break;
