...
int get_note_from_frequency(int frequency, unsigned int c5speed)
{
	...
	for (n = 0; n <= 120; n++) {
		...
		if (frequency <= get_frequency_from_note(n + 1, c5speed)) return n + 1;
	}
	return 120;
}
...
void fx_key_off(song_t *csf, uint32_t nchan)
{
	...
	if ((chan->flags & CHN_SUSTAINLOOP) && chan->ptr_sample) {
		...
		if (psmp->flags & CHN_LOOP) {
			if (psmp->flags & CHN_PINGPONGLOOP) chan->flags |= CHN_PINGPONGLOOP;
			else chan->flags &= ~(CHN_PINGPONGLOOP | CHN_PINGPONGFLAG);
			chan->flags |= CHN_LOOP;
			...
			if (chan->position >= chan->length) chan->position = chan->position - chan->length + chan->loop_start;
		} else {
			chan->flags &= ~(CHN_LOOP | CHN_PINGPONGLOOP | CHN_PINGPONGFLAG);
			chan->length = psmp->length;
		}
	}
	if (penv && penv->fadeout && (penv->flags & ENV_VOLLOOP)) chan->flags |= CHN_NOTEFADE;
}
...
int32_t csf_fx_do_freq_slide(uint32_t flags, int32_t frequency, int32_t slide, int is_tone_portamento)
{
	...
	if (flags & SONG_LINEARSLIDES) {
		...
		if (slide > 0) {
			if (n < 16) frequency = _muldivr(frequency, fine_linear_slide_up_table[n], 65536);
			else frequency = _muldivr(frequency, linear_slide_up_table[n / 4], 65536);
			if (old_frequency == frequency) frequency++;
		} else if (slide < 0) {
			if (n < 16) frequency = _muldivr(frequency, fine_linear_slide_down_table[n], 65536);
			else frequency = _muldivr(frequency, linear_slide_down_table[n / 4], 65536);
			if (old_frequency == frequency) frequency--;
		}
	} else {
		if (slide < 0) {
			...
			if (frequency_div <= 0) {
				if (is_tone_portamento) frequency_div = 1;
				else return 0;
			}
			int64_t freq = ((1712 * 8363 * (int64_t)frequency) / frequency_div);
			if (freq > INT32_MAX) frequency = INT32_MAX;
			else frequency = (int32_t)freq;
		}
	}
	return frequency;
}
...
static void set_instrument_panning(song_voice_t *chan, int32_t panning)
{
	...
	if (chan->flags & CHN_SURROUND) chan->channel_panning |= 0x8000;
...
{
	if (!(flags & SONG_FIRSTTICK)) chan->frequency = csf_fx_do_freq_slide(flags, chan->frequency, (int)(param * 4), 0);
}
...
{
	if (!param) param = chan->mem_pitchslide;

	switch (param & 0xf0) {
	case 0xe0: fx_extra_fine_portamento_up(flags, chan, param & 0x0F); break;
	case 0xf0: fx_fine_portamento_up(flags, chan, param & 0x0F); break;
	default: fx_reg_portamento_up(flags, chan, param); break;
	}
}
...
static void fx_portamento_down(uint32_t flags, song_voice_t *chan, uint32_t param)
{
	...
	switch (param & 0xf0) {
	case 0xe0: fx_extra_fine_portamento_down(flags, chan, param & 0x0F); break;
	case 0xf0: fx_fine_portamento_down(flags, chan, param & 0x0F); break;
	default: fx_reg_portamento_down(flags, chan, param); break;
	}
}

static void fx_tone_portamento(uint32_t flags, song_voice_t *chan, uint32_t param)
{
	if (!param) param = chan->mem_portanote;
...
static void fx_note_slide(uint32_t flags, song_voice_t *chan, uint32_t param, int sign)
{
	...
	if (flags & SONG_FIRSTTICK) {
		x = param & 0xf0;
		if (x) chan->note_slide_speed = (x >> 4);
		y = param & 0xf;
		if (y) chan->note_slide_step = y;
		...
		if (--chan->note_slide_counter == 0) {
			...
			chan->frequency = get_frequency_from_note(
				sign * chan->note_slide_step + get_note_from_frequency(chan->frequency, chan->c5speed), chan->c5speed);
		}
	}
}
...
{
	if (param & 0x0F) p->vibrato_depth = (param & 0x0F) * 4;
	if (param & 0xF0) p->vibrato_speed = (param >> 4) & 0x0F;
	p->flags |= CHN_VIBRATO;
}

static void fx_fine_vibrato(song_voice_t *p, uint32_t param)
{
	if (param & 0x0F) p->vibrato_depth = param & 0x0F;
	if (param & 0xF0) p->vibrato_speed = (param >> 4) & 0x0F;
	p->flags |= CHN_VIBRATO;
}
...
static void fx_panbrello(song_voice_t *chan, uint32_t param)
{
	...
	if (param & 0x0F) chan->panbrello_depth = param & 0x0F;
	if (param & 0xF0) chan->panbrello_speed = (param >> 4) & 0x0F;

	switch (chan->panbrello_type) {
	case VIB_SINE:
	default: pdelta = sine_table[panpos]; break;
	case VIB_RAMP_DOWN: pdelta = ramp_down_table[panpos]; break;
	case VIB_SQUARE: pdelta = square_table[panpos]; break;
	case VIB_RANDOM: pdelta = (rand() & 0x7F) - 0x40; break;
	}
...
{
	chan->volume += param * 4;
	if (chan->volume > 256) chan->volume = 256;
}
...
static void fx_volume_slide(uint32_t flags, song_voice_t *chan, uint32_t param)
{
	...
	if (param) chan->mem_volslide = param;
	else param = chan->mem_volslide;
	...
	if (param == (param & 0xf0)) {
		...
		param >>= 4;
		if (param == 0xf || !(flags & SONG_FIRSTTICK)) fx_volume_up(chan, param);
...
{
	int32_t slide = 0;
	if (param) chan->mem_panslide = param;
	else param = chan->mem_panslide;
	if ((param & 0x0F) == 0x0F && (param & 0xF0)) {
		...
		if (!(flags & SONG_FIRSTTICK)) {
			if (param & 0x0F) slide = (int)((param & 0x0F) << 2);
			else slide = -(int)((param & 0xF0) >> 2);
		}
	}
	if (slide) {
		slide += chan->panning;
...
static void fx_tremolo(uint32_t flags, song_voice_t *chan, uint32_t param)
{
	...
	if (param & 0x0F) chan->tremolo_depth = (param & 0x0F) << 2;
	if (param & 0xF0) chan->tremolo_speed = (param >> 4) & 0x0F;

	chan->flags |= CHN_TREMOLO;

	// don't handle on first tick if old-effects mode
	if ((flags & SONG_FIRSTTICK) && (flags & SONG_ITOLDEFFECTS)) return;

	switch (chan->tremolo_type) {
	case VIB_SINE:
	default: tdelta = sine_table[trempos]; break;
	case VIB_RAMP_DOWN: tdelta = ramp_down_table[trempos]; break;
	case VIB_SQUARE: tdelta = square_table[trempos]; break;
	case VIB_RANDOM: tdelta = 128 * ((double)rand() / RAND_MAX) - 64; break;
	}
...
static void fx_retrig_note(song_t *csf, uint32_t nchan, uint32_t param)
{
	...
	if ((csf->flags & SONG_FIRSTTICK) && chan->row_note != NOTE_NONE) {
		...
		if (chan->position == 0) return;

		chan->cd_retrig = param & 0xf;
		param >>= 4;
		if (param) {
			int vol = chan->volume;
			if (retrig_table_1[param]) vol = (vol * retrig_table_1[param]) >> 4;
			else vol += (retrig_table_2[param]) << 2;
		...
		if (NOTE_IS_NOTE(note) && chan->length) csf_check_nna(csf, nchan, 0, note, 1);
		csf_note_change(csf, nchan, note, 1, 1, 0);
		if (frequency && chan->row_note == NOTE_NONE) chan->frequency = frequency;
...
{
	int32_t slide = 0;
	if (param) chan->mem_channel_volslide = param;
	else param = chan->mem_channel_volslide;
	if ((param & 0x0F) == 0x0F && (param & 0xF0)) {
		if (flags & SONG_FIRSTTICK) slide = param >> 4;
	} else if ((param & 0xF0) == 0xF0 && (param & 0x0F)) {
		if (flags & SONG_FIRSTTICK) slide = -(int)(param & 0x0F);
	} else {
		if (!(flags & SONG_FIRSTTICK)) {
			if (param & 0x0F) slide = -(int)(param & 0x0F);
			else slide = (int)((param & 0xF0) >> 4);
		}
	}
	if (slide) {
...
{
	int32_t slide = 0;
	if (param) chan->mem_global_volslide = param;
	else param = chan->mem_global_volslide;
	if ((param & 0x0F) == 0x0F && (param & 0xF0)) {
		if (csf->flags & SONG_FIRSTTICK) slide = param >> 4;
	} else if ((param & 0xF0) == 0xF0 && (param & 0x0F)) {
		if (csf->flags & SONG_FIRSTTICK) slide = -(int)(param & 0x0F);
	} else {
		if (!(csf->flags & SONG_FIRSTTICK)) {
			if (param & 0xF0) slide = (int)((param & 0xF0) >> 4);
			else slide = -(int)(param & 0x0F);
		}
	}
	if (slide) {
...
static void fx_special(song_t *csf, uint32_t nchan, uint32_t param)
{
	...
	case 0x30: chan->vib_type = param; break;
	// S4x: Set Tremolo WaveForm
	case 0x40: chan->tremolo_type = param; break;
	...
	case 0x70:
		if (!(csf->flags & SONG_FIRSTTICK)) break;
		switch (param) {
		case 0:
		case 1:
		case 2: {
	...
	// SBx: Pattern Loop
	case 0xB0:
		if (csf->flags & SONG_FIRSTTICK) fx_pattern_loop(csf, chan, param & 0x0F);
		break;
	// SCx: Note Cut
	case 0xC0:
		if (csf->flags & SONG_FIRSTTICK) chan->cd_note_cut = param ? param : 1;
		else if (--chan->cd_note_cut == 0) fx_note_cut(csf, nchan, 1);
		break;
	// SDx: Note Delay
	...
	case 0xF0: chan->active_macro = param; break;
	}
}
...
void csf_process_midi_macro(
	song_t *csf, uint32_t nchan, const char *macro, uint32_t param, uint32_t note, uint32_t velocity,
	uint32_t use_instr)
{
	/* this was all wrong. -mrsb */
	song_voice_t *chan = &csf->voices[nchan];
	song_instrument_t *penv = ((csf->flags & SONG_INSTRUMENTMODE) && chan->last_instrument < MAX_INSTRUMENTS) ?
	...
	for (int read_pos = 0; read_pos <= 32 && macro[read_pos]; read_pos++) {
		...
		case 'u': {
			...
			if (!(chan->flags & CHN_MUTE)) data = (unsigned char)CLAMP(chan->final_volume >> 7, 0x01, 0x7F);
			break;
		}
		case 'x':
			/* Panning */
		...
		case 'a':
			/* MIDI Bank (high byte) */
			if (penv && penv->midi_bank != -1) data = (unsigned char)((penv->midi_bank >> 7) & 0x7F);
			break;
		case 'b':
			/* MIDI Bank (low byte) */
			if (penv && penv->midi_bank != -1) data = (unsigned char)(penv->midi_bank & 0x7F);
			break;
		case 'p':
			/* MIDI Program */
			if (penv && penv->midi_program != -1) data = (unsigned char)(penv->midi_program & 0x7F);
			break;
		case 'z':
			/* Zxx Param */
...
unsigned int csf_get_length(song_t *csf)
{
	...
	for (;;) {
		...
		if (csf->stop_at_order > -1 && csf->stop_at_row > -1) {
			if (csf->stop_at_order <= (signed)cur_order && csf->stop_at_row <= (signed)row) break;
		...
		if (setloop) {
			for (n = 0; n < MAX_CHANNELS; n++)
				if (setloop & (1 << n)) patloop[n] = elapsed;
			setloop = 0;
		}
		...
		for (n = 0; n < MAX_CHANNELS; note++, n++) {
			...
			case FX_PATTERNBREAK:
				...
				break;
			case FX_SPEED:
				if (param) speed = param;
				break;
			case FX_TEMPO:
				if (param) mem_tempo[n] = param;
				else param = mem_tempo[n];
				int d = (param & 0xf);
				switch (param >> 4) {
				default: tempo = param; break;
				case 0: d = -d;
				case 1:
					d = d * (speed - 1) + tempo;
			...
			case FX_SPECIAL:
				switch (param >> 4) {
				case 0x6: speed_count = param & 0x0F; break;
				case 0xb:
					if (param & 0x0F) {
					...
					break;
				case 0xe: speed_count = (param & 0x0F) * speed; break;
				}
				break;
			}
		}
...
static void env_reset(song_voice_t *chan, int always)
{
	if (chan->ptr_instrument) {
		...
		if (always) {
			...
			if (!(chan->ptr_instrument->flags & ENV_VOLCARRY)) chan->vol_env_position = 0;
			if (!(chan->ptr_instrument->flags & ENV_PANCARRY)) chan->pan_env_position = 0;
			if (!(chan->ptr_instrument->flags & ENV_PITCHCARRY)) chan->pitch_env_position = 0;
		}
	}
...
void csf_instrument_change(song_t *csf, song_voice_t *chan, uint32_t instr, int porta, int inst_column)
{
	...
	*/
	if (penv && !inst_changed && psmp != oldsmp && chan->ptr_sample && !NOTE_IS_NOTE(chan->row_note)) return;
	...
	if (penv) {
		if ((!chan->length) || (inst_column && porta && (csf->flags & SONG_COMPATGXX))
		    || (inst_column && !porta && (chan->flags & (CHN_NOTEFADE | CHN_KEYOFF))
	...
	if (porta && !chan->length) chan->increment = 0;

	chan->flags &= ~(CHN_SAMPLE_FLAGS | CHN_KEYOFF | CHN_NOTEFADE | CHN_VOLENV | CHN_PANENV | CHN_PITCHENV);
	if (penv) {
		if (penv->flags & ENV_VOLUME) chan->flags |= CHN_VOLENV;
		if (penv->flags & ENV_PANNING) chan->flags |= CHN_PANENV;
		if (penv->flags & ENV_PITCH) chan->flags |= CHN_PITCHENV;
		if (penv->ifc & 0x80) chan->cutoff = penv->ifc & 0x7F;
		if (penv->ifr & 0x80) chan->resonance = penv->ifr & 0x7F;
	}

	if (chan->row_note == NOTE_OFF && (csf->flags & SONG_ITOLDEFFECTS) && psmp != oldsmp) {
		if (chan->ptr_sample) chan->flags |= chan->ptr_sample->flags & CHN_SAMPLE_FLAGS;
		if (psmp->flags & CHN_PANNING) chan->panning = psmp->panning;
	...
	if ((chan->flags & CHN_LOOP) && chan->loop_end < chan->length) chan->length = chan->loop_end;
...
void csf_note_change(song_t *csf, uint32_t nchan, int note, int porta, int retrig, int have_inst)
{
	...
	if (note == NOTE_NONE || note < 0) return;
	...
	song_instrument_t *penv = (csf->flags & SONG_INSTRUMENTMODE) ? chan->ptr_instrument : NULL;
	if (penv && NOTE_IS_NOTE(note)) {
		if (!penv->sample_map[note - 1]) return;
		if (!(have_inst && porta && pins)) pins = csf_translate_keyboard(csf, penv, note, pins);
	...
	if (NOTE_IS_CONTROL(note)) {
		...
		case NOTE_OFF:
			...
			break;
		case NOTE_CUT: fx_note_cut(csf, nchan, 1); break;
		case NOTE_FADE:
		default: // Impulse Tracker handles all unknown notes as fade internally
			if (csf->flags & SONG_INSTRUMENTMODE) chan->flags |= CHN_NOTEFADE;
			break;
		}
		return;
	}
	...
	if (!porta && pins) chan->c5speed = pins->c5speed;

	if (porta && !chan->increment) porta = 0;
	...
	if (frequency) {
		...
		if (chan->position >= chan->length) chan->position = chan->loop_start;
	} else {
		porta = 0;
	}
	...
	if (porta && (csf->flags & SONG_COMPATGXX && chan->row_instr)) chan->flags &= ~CHN_KEYOFF;

	// Enable Ramping
	if (!porta) {
		chan->vu_meter = 0x0;
		chan->strike =
			4; /* this affects how long the initial hit on the playback marks lasts (bigger dot in instrument and sample list windows)*/
...
uint32_t csf_get_nna_channel(song_t *csf, uint32_t nchan)
{
	...
	for (uint32_t j = MAX_CHANNELS; j < MAX_VOICES; j++, pj++) {
		...
		if (pj->flags & CHN_NOTEFADE) v = v * pj->fadeout_volume;
		else v <<= 16;
...
void csf_check_nna(song_t *csf, uint32_t nchan, uint32_t instr, int note, int force_cut)
{
	...
	signed char *data;
	if (!NOTE_IS_NOTE(note)) return;
	// Always NNA cut - using
	if (force_cut || !(csf->flags & SONG_INSTRUMENTMODE)) {
		if (!chan->length || (chan->flags & CHN_MUTE) || (!chan->left_volume && !chan->right_volume)) return;
	...
	if (ptr_instrument != NULL) {
		...
		if (n > 0 && n < MAX_SAMPLES) data = csf->samples[n].data;
		else /* OpenMPT test case emptyslot.it */ return;
	}
	if (!penv) return;
	p = chan;
	for (uint32_t i = nchan; i < MAX_VOICES; p++, i++) {
		if (!((i >= MAX_CHANNELS || p == chan) && ((p->master_channel == nchan + 1 || p == chan) && p->ptr_instrument)))
			continue;
		int apply_dna = 0;
		...
		case DCT_SAMPLE:
			...
			break;
		case DCT_INSTRUMENT: apply_dna = (ptr_instrument == p->ptr_instrument); break;
		}

		// Duplicate Note Action
		if (apply_dna) {
			switch (p->ptr_instrument->dca) {
			case DCA_NOTECUT: fx_note_cut(csf, i, 1); break;
			case DCA_NOTEOFF: fx_key_off(csf, i); break;
			case DCA_NOTEFADE: p->flags |= CHN_NOTEFADE; break;
			}
			if (!p->volume) {
				p->fadeout_volume = 0;
				p->flags |= (CHN_NOTEFADE | CHN_FASTVOLRAMP);
	...
	if (chan->increment && chan->length) {
		...
		if (n) {
			...
			*p = *chan;
			...
			// Key Off the note
			switch (chan->nna) {
			case NNA_NOTEOFF: fx_key_off(csf, n); break;
			case NNA_NOTECUT: p->fadeout_volume = 0;
...
static void handle_effect(song_t *csf, uint32_t nchan, uint32_t cmd, uint32_t param, int porta, int firsttick)
{
	...
	case FX_PORTAMENTOUP:
		if (firsttick) {
			if (param) chan->mem_pitchslide = param;
			if (!(csf->flags & SONG_COMPATGXX)) chan->mem_portanote = chan->mem_pitchslide;
		}
	...
	case FX_VOLUMESLIDE: fx_volume_slide(csf->flags | (firsttick ? SONG_FIRSTTICK : 0), chan, param); break;

	case FX_TONEPORTAMENTO:
		if (firsttick) {
			if (param) chan->mem_portanote = param;
			if (!(csf->flags & SONG_COMPATGXX)) chan->mem_pitchslide = chan->mem_portanote;
		}
	...
	case FX_VIBRATO: fx_vibrato(chan, param); break;
	...
	case FX_TEMPO:
		if (csf->flags & SONG_FIRSTTICK) {
			if (param) chan->mem_tempo = param;
			else param = chan->mem_tempo;
			if (param >= 0x20) csf->current_tempo = param;
		} else {
			...
			case 0:
				...
				if (csf->current_tempo < 32) csf->current_tempo = 32;
				break;
			case 1:
				csf->current_tempo += param & 0xf;
				if (csf->current_tempo > 255) csf->current_tempo = 255;
				break;
			}
		}
		break;
	...
	case FX_OFFSET:
		if (!(csf->flags & SONG_FIRSTTICK)) break;
		if (param) chan->mem_offset = (chan->mem_offset & ~0xff00) | (param << 8);
	...
	case FX_ARPEGGIO:
		...
		if (!(csf->flags & SONG_FIRSTTICK)) break;
		if (param) chan->mem_arpeggio = param;
		break;

	case FX_RETRIG:
		if (param) chan->mem_retrig = param & 0xFF;
	...
	case FX_TREMOR:
		...
		if (csf->flags & SONG_FIRSTTICK) {
			if (!param) param = chan->mem_tremor;
		...
		if ((chan->cd_tremor & 128) && chan->length) {
			if (chan->cd_tremor == 128) chan->cd_tremor = (chan->mem_tremor >> 4) | 192;
			else if (chan->cd_tremor == 192) chan->cd_tremor = (chan->mem_tremor & 0xf) | 128;
			else chan->cd_tremor--;
		}
	...
	case FX_GLOBALVOLUME:
		if (!firsttick) break;
		if (param <= 128) csf->current_global_volume = param;
		break;

	case FX_GLOBALVOLSLIDE: fx_global_vol_slide(csf, chan, param); break;
	...
	case FX_PANNINGSLIDE: fx_panning_slide(csf->flags | (firsttick ? SONG_FIRSTTICK : 0), chan, param); break;

	case FX_TREMOLO: fx_tremolo(csf->flags | (firsttick ? SONG_FIRSTTICK : 0), chan, param); break;

	case FX_FINEVIBRATO: fx_fine_vibrato(chan, param); break;

	case FX_SPECIAL: fx_special(csf, nchan, param); break;

	case FX_KEYOFF:
		if ((csf->current_speed - csf->tick_count) == param) fx_key_off(csf, nchan);
		break;
	...
	case FX_CHANNELVOLSLIDE: fx_channel_vol_slide(csf->flags | (firsttick ? SONG_FIRSTTICK : 0), chan, param); break;

	case FX_PANBRELLO: fx_panbrello(chan, param); break;
	...
	case FX_POSITIONJUMP:
		if (!(csf->mix_flags & SNDMIX_NOBACKWARDJUMPS) || csf->process_order < param) csf->process_order = param - 1;
	...
	case FX_MIDI: {
		...
		const uint32_t vel = !chan->ptr_sample ? 0 :
		                                         _muldiv(
													 chan->volume * csf->current_global_volume * chan->global_volume,
													 chan->ptr_sample->global_volume * 2, 1 << 21);

		csf_process_midi_macro(
			csf, nchan, (param < 0x80) ? csf->midi_config.sfx[chan->active_macro] : csf->midi_config.zxx[param & 0x7F],
			param, chan->note, vel, 0);
		break;
	}

	case FX_NOTESLIDEUP: fx_note_slide(csf->flags | (firsttick ? SONG_FIRSTTICK : 0), chan, param, 1); break;
	case FX_NOTESLIDEDOWN: fx_note_slide(csf->flags | (firsttick ? SONG_FIRSTTICK : 0), chan, param, -1); break;
	}
}

static void
handle_voleffect(song_t *csf, song_voice_t *chan, uint32_t volcmd, uint32_t vol, int firsttick, int start_note)
{
	...
	case VOLFX_PORTAUP: // Fx
		if (start_note) {
			if (vol) chan->mem_pitchslide = 4 * vol;
			if (!(csf->flags & SONG_COMPATGXX)) chan->mem_portanote = chan->mem_pitchslide;
		} else {
	...
	case VOLFX_TONEPORTAMENTO: // Gx
		if (start_note) {
			if (vol) chan->mem_portanote = vc_portamento_table[vol & 0x0F];
			if (!(csf->flags & SONG_COMPATGXX)) chan->mem_pitchslide = chan->mem_portanote;
		} else {
	...
	case VOLFX_VOLSLIDEUP: // Cx
		if (start_note) {
			if (vol) chan->mem_vc_volslide = vol;
		} else {
...
void csf_process_effects(song_t *csf, int firsttick)
{
	...
	for (uint32_t nchan = 0; nchan < MAX_CHANNELS; nchan++, chan++) {
		...
		int porta = (cmd == FX_TONEPORTAMENTO || cmd == FX_TONEPORTAVOL || volcmd == VOLFX_TONEPORTAMENTO);
		...
		if (csf->flags & SONG_INSTRUMENTMODE && instr > 0 && instr < MAX_INSTRUMENTS
		...
		if (cmd == FX_SPECIAL) {
			if (param) chan->mem_special = param;
			else param = chan->mem_special;
			if (param >> 4 == 0xd) {
				...
				if (--chan->cd_note_delay > 0) continue; // not our turn yet, go away
		...
		if (start_note) {
			...
			if (csf->flags & SONG_INSTRUMENTMODE && (NOTE_IS_NOTE(note) || note == NOTE_NONE)) {
				...
				if (instrcheck
				    && (instrcheck < 0 || instrcheck > MAX_INSTRUMENTS || csf->instruments[instrcheck] == NULL)) {
					note = NOTE_NONE;
					instr = 0;
				}
			}
			if (csf->flags & SONG_INSTRUMENTMODE && instr && !NOTE_IS_NOTE(note)) {
				if ((porta && csf->flags & SONG_COMPATGXX) || (!porta && csf->flags & SONG_ITOLDEFFECTS)) {
					env_reset(chan, 1);
					chan->fadeout_volume = 65536;
				}
			}
			...
			if (instr && note == NOTE_NONE) {
				if (csf->flags & SONG_INSTRUMENTMODE) {
					if (chan->ptr_sample) chan->volume = chan->ptr_sample->volume;
			...
			if (NOTE_IS_CONTROL(note)) {
				if (instr) {
					...
					if (csf->flags & SONG_INSTRUMENTMODE) {
						smp = 0;
						if (csf->instruments[instr]) smp = csf->instruments[instr]->sample_map[chan->note];
					}
					if (smp > 0 && smp < MAX_SAMPLES) chan->volume = csf->samples[smp].volume;
				}

				if (!(csf->flags & SONG_ITOLDEFFECTS)) instr = 0;
			}
			...
			if (NOTE_IS_NOTE(note)) {
				...
				if (!porta) csf_check_nna(csf, nchan, instr, note, 0);

				if (chan->channel_panning > 0) {
					chan->panning = (chan->channel_panning & 0x7FFF) - 1;
					if (chan->channel_panning & 0x8000) chan->flags |= CHN_SURROUND;
					chan->channel_panning = 0;
				}
			}
			// Instrument Change ?
			if (instr) {
				...
				if ((csf->flags & SONG_INSTRUMENTMODE) && csf->instruments[instr])
					GM_DPatch(
						nchan, csf->instruments[instr]->midi_program, csf->instruments[instr]->midi_bank,
			...
			if (note != NOTE_NONE) {
				if (!instr && chan->new_instrument && NOTE_IS_NOTE(note)) {
					...
					chan->new_note = note;
					if ((csf->flags & SONG_INSTRUMENTMODE) && chan->new_instrument < MAX_INSTRUMENTS
						...
						GM_DPatch(
							nchan, csf->instruments[chan->new_instrument]->midi_program,
