...
`http://127.0.0.1:8080` と `http://localhost:8080` はブラウザ上で別オリジン扱いのため、IndexedDB（IDBFS の永続領域）も分かれます。挙動の切り分けでは **`localhost` で開く**とよいです。
...
- `Fit Window`: ヘッダー下の利用可能領域に **640×400 の縦横比を保ったまま**最大表示（レターボックス）。デフォルト。
- `x1`: 固定 `640x400`（論理ピクセルに等倍の CSS サイズ）
- `x2`: 固定 `1280x800`
- `x3`: 固定 `1920x1200`

トラッカー描画用キャンバスは **画面左寄せ**（中央揃えではない）です。
